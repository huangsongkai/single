...
<%
		...
		String zpag_sql="SELECT COUNT(1) as row "+
						"FROM (SELECT  orderid ,MAX(STATUS) STATUS,processid,nodeid  FROM process_log WHERE  rolecode='industrysign' AND operatorid='"+Suid+"' AND  process_log.regionalcode LIKE '1000:1%'   GROUP BY orderid)process_log "+
        ...
        String orderid="",process="",nodeid="",models="";
...
System.out.println("customer_sql===="+customer_sql);
          	...
          	while(customerPrs.next()){
          		  ...
          		  models=customerPrs.getString("models");
		          list.add("<tr>\r\n"+
							...
							"<td class=\"\">\r\n"+
							 "<div class=\"layui-btn-group\">\r\n"+
							    "<button class=\"layui-btn\"  onclick=\" newfile('"+customerPrs.getString("orderid")+"','"+customerPrs.getString("nodeid")+"','"+customerPrs.getString("processid")+"')\">编辑</button>\r\n"+
							  "</div>\r\n"+
							"</td>\r\n"+
					      "</tr>\r\n");
		        i++;
...
<html>
	...
	<body>
	    ...
	    <script type="text/javascript">   
		     function newfile(orderid,nodeid,process){
				layer.open({
				  type: 2,
				  offset:'t',
				  title: '接单',
				  ...
				  shade: 0.5,
				  area: ['980px', '530px'],
				  content: 'receive.jsp?orderid='+orderid+'&nodeid='+nodeid+'&process='+process
				}); 
		     }
		     function ac_tion(ac) {
			       if(ac=='refresh'){//刷新
