...
<%
		//判断当前角色是否匹配
			...
			System.out.println("=====================");
			System.out.println("Srolecode=="+Srolecode);
			System.out.println("=====================");
		...
		//计算出总页数	 
		int zpag= db.Row("SELECT COUNT(1) as row FROM process_log LEFT JOIN order_sheet ON process_log.orderid=order_sheet.id LEFT JOIN order_customerfile ON order_customerfile.id= order_sheet.customeruid  WHERE process_log.rolecode='industrysign' AND  STATUS=6 AND process_log.regionalcode LIKE '"+Sregionalcode+"%' "+search+";");
		zpag=(zpag+10-1)/10;
        	...
        	if("".equals(ac)){//默认首页  （倒序）
        			System.out.println("默认首页  ");
	          	 	customer_sql = "SELECT "+
									...
									"WHERE   rolecode='industrysign' "+
										"AND  STATUS=6 "+
										"AND process_log.regionalcode LIKE '"+Sregionalcode+"%' "+
										search+
		          ...
		          list.add("<tr>\r\n"+
							...
							"<td class=\"\">\r\n"+
							 "<div class=\"layui-btn-group\">\r\n"+
							    "<button class=\"layui-btn\"  onclick=\" newfile('"+customerPrs.getString("orderid")+"')\">查看</button>\r\n"+
							  "</div>\r\n"+
							"</td>\r\n"+
					      "</tr>\r\n");
		        i++;
	    ...
	    <script type="text/javascript">   
		     function newfile(orderid){
				layer.open({
				  ...
				  shade: 0.5,
				  area: ['980px', '530px'],
				  content: 'insiderSignature_check.jsp?orderid='+orderid 
				}); 
		     }
		     function ac_tion(ac) {
			       if(ac=='refresh'){//刷新
