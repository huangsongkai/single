	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        ...
		        <%
		        	...
		        	if("".equals(ac)){//默认首页  （倒序）
						...
						//				+"  	order_sheet,                                                  "
						//				+"  	order_customerfile                                            "
						//				+"	WHERE order_customerfile.customername LIKE '%%'                   "
						//				+"    AND (temp1.status = '0'  )                "
		          ...
		          <tr>
					...
					<input type="hidden" value="<%=customerPrs.getString("processid") %>" name="processid" id="processid" />
					 <div class="layui-btn-group">
					  <%if("0".equals(customerPrs.getString("status"))){ %>  <button class="layui-btn" onclick="buttonin('<%=customerPrs.getString("orderid") %>')">接单</button><%} %>
					  </div>
					</td>
			      </tr>
			    <%i++;}%>
