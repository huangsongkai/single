...
<html>
	...
	<body>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="预约时间"  data-sortable="true" data-filter-control="select"  data-visible="true">预约时间</th>
		        ...
		        <%
		        	...
		        	if("".equals(ac)){//默认首页  （倒序）
		        			customer_sql = 	"	SELECT t4.*, IFNULL(max(t5.appointmenttime),'暂无' ) appointmenttime from "
													+"				(SELECT                                      "
													+"						t1.*, t2.customeruid,                "
													+"							t2.salesmanname,                 "
													+"							t2.salesmanid,                   "
													...
													+"					) `t1`,                                  "
													+"					order_sheet t2,                          "
													+"					order_customerfile t3                    "
													+"				WHERE                                        "
													+"					t3.customername LIKE '%%'                "
													+"				AND (                                        "
													+"					t1. STATUS = '1'                         "
													+"				)                                            "
													+"				AND t2.customeruid = t3.id                   "
													+"				AND t1.orderid = t2.id                       "
													+"				AND operatorid = "+Suid+"              "
													+"				AND t1.regionalcode LIKE  '"+user.getRegionalcode()+"%'                           "
													+"				AND t1.rolecode =  '"+user.getRolecode()+"'                              "
													+"				) t4  left join	homevisits_time t5 on t5.orderid = t4.orderid                   "
													+"				GROUP BY t4.orderid                          ";
						//customer_sql = "SELECT *															  "
						//				+"FROM (SELECT *                                                      "
						//				+"      FROM (SELECT *                                                "
		        ...
		        %>
		          <tr>
					...
					<td class=""><%=customerPrs.getString("appointmenttime") %></td>
					<td class=""><%=customerPrs.getString("updatetime") %></td>
					<td class=""><font color="red"><%=common.getDis4info("state",customerPrs.getString("status")) %></font></td>
					<td class="">
					...
					<input type="hidden" value="<%=customerPrs.getString("orderid") %>" name="orderid" id="orderid" />
					 <div class="layui-btn-group">
					    ...
					    <button class="layui-btn" onclick="bianji('<%=customerPrs.getString("orderid") %>','<%=customerPrs.getString("nodeid") %>','<%=customerPrs.getString("processid") %>')" >编辑</button>
					  </div>
					</td>
			      </tr>
			    <%i++;}%>
	    ...
	    <script type="text/javascript">   
	    	function bianji(orderid,nodeid,process){
	    		layer.open({
				  type: 2,
				  title: '编辑',
				  shadeClose: true,
				  ...
				  shade: 0.5,
				  area: ['980px', '500px'],
				  content: 'visitier_ti.jsp?orderid='+orderid+'&nodeid='+nodeid+'&process='+process 
