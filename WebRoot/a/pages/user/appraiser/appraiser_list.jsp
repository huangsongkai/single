	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="">
		        <input id="search" type="text" class="layui-input textbox-text" placeholder="输入订单编号进行查询" style="">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="newfile('newfile')" >新增报件</button>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        ...
		        <%
		        	...
		        	if("".equals(ac)){//默认首页  （倒序）
			          	 	...
			          	 	customer_sql = "SELECT order_sheet.ordercode as ordercode ,"+//订单编号
			          								  "order_sheet.loantype as loantype,"+//贷款类别
			          								  "g_polocy.policyname as policyname,"+//收费政策
			          								  "order_customerfile.customername as customername ,"+//姓名
			          								  "order_customerfile.contactaddress as contactaddress,"+//家庭住址
			          								  "order_customerfile.phonenumber as phonenumber ,"+//联系电话
			          								  "order_sheet.updatetime as updatetime "+//联系电话
												"FROM  order_sheet "+
													"LEFT JOIN order_customerfile ON order_sheet.customeruid=order_customerfile.id "+
													"LEFT JOIN g_polocy ON g_polocy.id = order_sheet.policyid "+
													"LEFT JOIN process_log ON process_log.orderid=order_sheet.id "+
													"LEFT JOIN zk_user_role ON zk_user_role.sys_user_id=process_log.operatorid "+
												"WHERE process_log.operatorid="+Suid+"  AND zk_user_role.sys_role_id=2 GROUP BY process_log.orderid HAVING COUNT(1)>1 ORDER BY order_sheet.id DESC;";
			          	 	...
			          	 	customer_sql = "SELECT order_sheet.ordercode as ordercode ,"+//订单编号
			          								  ...
			          								  "order_customerfile.contactaddress as contactaddress,"+//家庭住址
			          								  "order_customerfile.phonenumber as phonenumber,"+//联系电话
			          								  "order_sheet.updatetime as updatetime "+//订单创建时间 
												"FROM  order_sheet "+
													"LEFT JOIN order_customerfile ON order_sheet.customeruid=order_customerfile.id "+
													"LEFT JOIN g_polocy ON g_polocy.id = order_sheet.policyid "+
													"LEFT JOIN process_log ON process_log.orderid=order_sheet.id "+
													"LEFT JOIN zk_user_role ON zk_user_role.sys_user_id=process_log.operatorid "+
												"WHERE process_log.operatorid="+Suid+"  AND zk_user_role.sys_role_id=2 AND order_sheet.ordercode  LIKE '%"+val+"%' "+
												"GROUP BY process_log.orderid HAVING COUNT(1)>1 "+
												"ORDER BY order_sheet.id DESC;";
