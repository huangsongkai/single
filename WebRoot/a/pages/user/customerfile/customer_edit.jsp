							...
							<div class="layui-input-inline">
							  <select name="sex" lay-verify="required" >
							  	<option value="">请选择</option>
					        		<% 
											String gender_sql="SELECT id,typename FROM TYPE  WHERE typegroupid=7;"; 
											ResultSet gender_rs= db.executeQuery(gender_sql);
											while(gender_rs.next()){
												out.println(" <option value='"+gender_rs.getString("id")+"'>"+gender_rs.getString("typename")+"</option>");
											}if(gender_rs!=null){gender_rs.close();}
									%>
							  </select>
							</div>
				    </div>
				    ...
				    <div class="layui-inline">
							...
							<div class="layui-input-inline">
							  	<select name="nation" class="nation" lay-verify="required">
								    <option value="">请选择</option>
								  	<%
										String nation_sql="SELECT id, typename FROM  type where typegroupid=5;";  
										ResultSet nation_rs= db.executeQuery(nation_sql);
										while(nation_rs.next()){
											out.println(" <option value='"+nation_rs.getString("id")+"'>"+nation_rs.getString("typename")+"</option>");
										}if(nation_rs!=null){nation_rs.close();}
									%>
							 	</select>
							</div>
				    </div>
				    <div class="layui-inline">
					        ...
					        <div class="layui-input-inline" >
					        	<select name="maritalstatus" lay-verify="required" lay-filter="Marriage" >
					        		<option value="">请选择</option>
					        		<% 
											String marriage_sql="SELECT id,typename FROM TYPE  WHERE typegroupid=2;"; 
											ResultSet marriage_rs= db.executeQuery(marriage_sql);
											while(marriage_rs.next()){
												out.println(" <option value='"+marriage_rs.getString("id")+"'>"+marriage_rs.getString("typename")+"</option>");
											}if(marriage_rs!=null){marriage_rs.close();}
									%>
					        	</select>
					        </div>
				    </div>
			  		...
			  		<div class="layui-inline">
							...
							<div class="layui-input-inline" >
							 	<select name="creditStatus" lay-verify="required">
							 		 <option value=''>请选择</option>
									 <%
											String creditStatus_sql="SELECT id,typename FROM TYPE WHERE typegroupid=6 ;"; 
											ResultSet creditStatus_rs= db.executeQuery(creditStatus_sql);
											while(creditStatus_rs.next()){
												out.println(" <option value='"+creditStatus_rs.getString("id")+"'>"+creditStatus_rs.getString("typename")+"</option>");
											}if(creditStatus_rs!=null){creditStatus_rs.close();}
									  %>
							    </select>
							</div>
				    </div>
				...
				<%
					if("post".equals(ac)){
						 ...
						 String customer_sql="INSERT INTO order_customerfile "+
															"(customername,sex,birthdate,identityid,phonenumber,contactnumber,contactaddress,companyname,companyaddress,companyphone,nation,industrycategory,maritalstatus,spousename,spouseidnum,spousephone,spousecompany,spousecomaddress,spousecomphone,marriagenum,creditStatus,creditBank,creationdate,creationid,updatetime,upuid)"+
														"VALUES "+
															"('"+customername+"','"+sex+"','"+birthdate+"','"+identityid+"','"+phonenumber+"','"+contactnumber+"','"+contactaddress+"','"+companyname+"','"+companyaddress+"','"+companyphone+"','"+nation+"','"+industrycategory+"','"+maritalstatus+"','"+spousename+"','"+spouseidnum+"','"+spousephone+"','"+spousecompany+"','"+spousecomaddress+"','"+spousecomphone+"','"+marriagenum+"','"+creditStatus+"','"+creditBank+"',NOW(),'"+userid+"',NOW(),'"+userid+"');";
					     int customerid=db.executeUpdateRenum(customer_sql);
					     
					     
					     //写入征信记录表语句
						 String customer_instert="INSERT INTO f_creditreport  ( customerid,orderid, creditReportCount, cratetime, createid, updatetime, updateid, jsontemplate)VALUES('"+customerid+"' ,'0', '"+Credit_report+"',  now(), '"+userid+"', now(), '"+userid+"', 'jsontemplate');";					
