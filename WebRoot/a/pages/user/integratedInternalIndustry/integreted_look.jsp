...
<%
		...
		while(from_rs.next()){
       	...
       	//当前页面数据
       	String sql = "select advancetype,remarks from f_internal_industry where orderid='"+orderid+"'";
       	ResultSet baseRes = db.executeQuery(sql);
       	String advancetype = "";
       	String remarks = "";
       	if(baseRes.next()){
       		advancetype = baseRes.getString("advancetype");
       		remarks = baseRes.getString("remarks");
       	}
		...
		<form class="layui-form mar_20" action="?ac=post&nodeid=<%=nodeid %>&process=<%=process %>&orderid=<%=orderid %>" method="post">
			<div class="layui-form-item inline" >
				...
				<!--垫款类型              newspaper_model     -->
					<select name="advancetype" lay-filter="baojian_style" lay-verify="" disabled="disabled">
						<option value="">请选择</option>
						 <%
					     	String sql1 = " SELECT id,liuchengname FROM t_yewumian WHERE  parentid =( SELECT ty.id FROM t_yewumian AS ty,order_sheet AS os WHERE os.processid = ty.id AND os.id="+orderid+")";
					     	ResultSet ba = db.executeQuery(sql1);
					     	while(ba.next()){
			...
			<div class="layui-form-item layui-form-text marbo" >
				...
				<div class="layui-input-block">
					<textarea name="remarks" id="hhh" placeholder="请输入备注" class="layui-textarea mar_20" readonly><%=remarks %></textarea>
				</div>
			</div>  
		</form>
	</div>		 
