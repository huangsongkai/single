...
<html>
	...
	<body>
		        ...
		        <%
		        	...
		        	if("".equals(ac)){//默认首页  （倒序）
						customer_sql = "SELECT *															  "
										...
										+"  	order_sheet,                                                  "
										+"  	order_customerfile                                            "
										+"	WHERE  temp1.status = '6' AND nodeid = 5                                    "
							...
							customer_sql = "SELECT *															  "
										...
										+"    AND temp1.status = '6'  AND nodeid = 5                           "
										+"    AND order_sheet.customeruid = order_customerfile.id             "
										+"    AND temp1.orderid = order_sheet.id                              "
										+" 	  AND operatorid = "+Suid+"              "
										+"    AND temp1.regionalcode LIKE '"+user.getRegionalcode()+"%'                           "
		          ...
		          <tr>
					...
					<input type="hidden" value="<%=customerPrs.getString("orderid") %>" name="orderid" id="orderid" />
					 <div class="layui-btn-group">
					    ...
					    <button class="layui-btn" onclick="check('<%=customerPrs.getString("orderid") %>','<%=customerPrs.getString("nodeid") %>','<%=customerPrs.getString("processid") %>')" >查看</button>
					  </div>
					</td>
			      </tr>
			    <%i++;}%>
	    ...
	    <script type="text/javascript">   
	    	function check(orderid,nodeid,process){
	    		layer.open({
				  type: 2,
				  title: '查看',
				  shadeClose: true,
				  ...
				  shade: 0.5,
				  area: ['980px', '500px'],
				  content: './bank_look.jsp?orderid='+orderid+'&nodeid='+nodeid+'&process='+process 
