...
function bottom_judgment(val,e,a){
  ...
  var LPB = window.LPB = window.LPB || {plugins:[],
    ...
    */
    LPB.plugins['form_name'] = function (active_component,leipiplugins) {
      ...
      //下移组件弹出框
      var topnum=0;
      topnum=parseFloat(popover.css("top"))+60;
      popover.css("top",topnum);
      //充填默认值
      var fromChinese= $("#from_id").attr('fromChinese');
      $('#fromChinese').val(fromChinese);
      var fromEnglish= $("#from_id").attr('fromEnglish');
      $('#fromEnglish').val(fromEnglish);
      ...
      $(popover).delegate(".btn-info", "click", function(e){
         e.preventDefault();//阻止元素发生默认的行为(例如,当点击提交按钮时阻止对表单的提交
          ...
          $.each(inputs, function(i,e){
              ...
              if(attr_name == 'fromChinese')
              {
                  $(leipiplugins).attr("value", attr_val);
                  $(leipiplugins).attr("fromChinese", attr_val);
                  active_component.find(".leipiplugins-orgvalue").text(attr_val);
                  
                  if(attr_val.replace(/(^s*)|(s*$)/g, "").length==0){
                  	  $(e).focus();//添加焦点
                  	  jrumble('该字段不能为空');
                  	  return false;
                  }
              }else if(attr_name=='fromEnglish'){
            	  $(leipiplugins).attr("fromEnglish", attr_val);
            	  bottom_judgment(attr_val,e,active_component);
              }
          });
      });
