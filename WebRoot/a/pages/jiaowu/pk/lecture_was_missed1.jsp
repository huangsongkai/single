...
//获取文件后面的对象 数据
 ...
 String  sql1 = "SELECT																												"+
		...
		"	  t.class_begins_weeks AS class_begins_weeks,                                                                            "+
		...
		"		WHERE 1=1 AND t.timetablestate=2 "+sqlwhere+"		";
...
<table border="1" width="100%" bordercolorlight="#588FC7" cellspacing="0" cellpadding="0" bordercolor="#19A094"   id="tab1">
  <tbody>
      ...
      <td width="115" height="24"><p align="center">漏课原因</p></td>
...
%>
    <tr>
      ...
      <td width="56" align="center"><%
      		...
      		String log_sql = "select common from arrage_log where arrage_coursesystem_id = '"+bgRs1.getString("id")+"' order by id desc limit 1;";
      		ResultSet ssst = db.executeQuery(log_sql);
      		if(ssst.next()){
      			out.println(ssst.getString("common"));
      		}if(ssst!=null){ssst.close();}
...
function ac_tion() {
	...
	window.location.href="?ac=&xueqi="+xueqi+"&banji="+banji+"&zhuanye="+zhuanye+"&laoshi="+laoshi+"&dict_departments_id="+dict_departments_id+"";
}
...
function appoint(val) {
	 layer.open({
		  ...
		  shade: 0.5,
		  area: ['100%', '100%'],
		  content: 'scheduling_appoint1.jsp?id='+val
	});
}	
