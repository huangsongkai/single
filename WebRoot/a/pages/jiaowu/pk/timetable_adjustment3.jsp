	    	...
	    	<div class="layui-form-item">
		    	<div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				      	<select lay-verType="tips" name="xueqi" id="xueqi" disabled="disabled" >
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				      	<select lay-verType="tips" lay-filter="teaching_area" id = "teaching_area" name="teaching_area">
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				      	<select lay-verType="tips" lay-filter="classroom" id="classroom" name="classroom">
				        </select>
				    </div>
			    </div>
...
layui.use(['form','layer','jquery'], function(){
	...
	form.on('radio(kbRadio)',function(data){
		if($('#classroom').val()!='0'){
			kebiao($('#classroom').val());
		}
	})
	
	//获取课表
	function kebiao(classroom){
		if(classroom!='0'){
			//获取学期
			var xueqi = $("#xueqi").val();
			if(xueqi=="0"){
			...
			var obj_str1 = {"xueqi":xueqi,"classroom":classroom,"zhouci":zhouci,"kebiaotype":kebiaotype};
			var obj1 = JSON.stringify(obj_str1)
			var ret_str1=PostAjx('../../../../Api/v1/?p=web/kebiao/getClassroomInfo',obj1,'<%=Suid%>','<%=Spc_token%>');
			obj1 = JSON.parse(ret_str1);
			console.log(obj1);
			if(obj1.success){
				$(".moren").each(function(){
					...
					for(var j=0;j<array1.length;j++){
						...
						for(var m =0 ;m<array2.length;m++){
							if(array2[m]=="0"){
								...
								$("#"+j+"_"+m+"").attr("arrage_coursesid",array[i].arrage_coursesid+"|"+array[i].teacherid+"|"+array[i].class_grade_id);				//234|4   排课信息id|老师id|班级id
								$("#"+j+"_"+m+"").append(str);
							}
						}
					}
	...
	function changeClass(arrage_coursesid_one,id_one,arrage_coursesid_two,id_two,inspect){
		var xueqi = $("#xueqi").val();
		var classroom = $("#classroom").val();		//班级
		var obj_str1 = {"arrage_coursesid_one":arrage_coursesid_one,"id_one":id_one,"arrage_coursesid_two":arrage_coursesid_two,"id_two":id_two,'semester':xueqi,'classroom':classroom,'inspect':inspect};
		var obj1 = JSON.stringify(obj_str1)
		
		var ret_str1=PostAjx('../../../../Api/v1/?p=web/kebiao/timetableAdjustmentClassroom',obj1,'<%=Suid%>','<%=Spc_token%>');
		return ret_str1;
	}
