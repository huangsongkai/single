...
<%
	...
	String qSql = "select * from arrage_course_teacher t where t.semester='"+semester+"' and t.teacher_id='"+teacherid+"' and t.weekly='"+weekly+"'";
...
<div id="box">
	<blockquote class="layui-elem-quote" style="padding: 5px">
		&nbsp;&nbsp;&nbsp;
		教师时间设置:<span style="margin-left: 15px;">[学年学期号：<%=semester %>]</span>
	</blockquote>
...
<%--				    <input type="button" name="xxx" value="设置" class="layui-btn">--%>
			  	...
			  	<div class="layui-inline">
				    <label class="layui-form-label">已设置教师</label>
				    <div class="layui-input-inline">
				      	<select lay-verType="tips" lay-filter="selecteacher" lay-search>
				          <option value="">全部</option>
				         <%
				         	String selectSql = "SELECT	t.teacher_id,t2.teacher_name,t2.teacher_number FROM	arrage_course_teacher t	LEFT JOIN teacher_basic t2 on t.teacher_id=t2.id WHERE	t.semester = '"+semester+"' GROUP BY t.teacher_id";
				         	ResultSet selectSqlRs = db.executeQuery(selectSql);
				         	while(selectSqlRs.next()){
				         		out.println("<option value='"+selectSqlRs.getString("teacher_id")+"'>["+selectSqlRs.getString("teacher_number")+"]"+selectSqlRs.getString("teacher_name")+"</option>");
				         	}
				         %>
				        </select>
				    </div>
	...
	form.on('radio', function(data){
	 ...
	 form.on('select(teacherid)', function(data){
		  ...
		  var weekly = $('#weekly').val();
		 	location.href ="class_teacher_setting.jsp?semester="+semester+"&teacherid="+teacherid+"&weekly="+weekly;
		  });
		  ...
		  teacherid = request.getParameter("teacherid");
			...
			for(int l =0;l<classinfos.length;l++){
				if(classinfos[l].split("\\|").length>2){
					...
					String checkSql = "select count(id) row from arrage_course_teacher t where t.section_id="+b[0]+" and week_id = "+b[1]+" and weekly = '"+weekly+"' and semester= '"+semester+"' and teacher_id='"+teacherid+"'" ;
					String sql ="";
					if(db.Row(checkSql)>0){
						sql = "update arrage_course_teacher set state="+b[2]+",totalid='"+classinfos[l]+"',remarks='"+remarks+"'  where section_id="+b[0]+" and week_id = "+b[1]+" and semester= '"+semester+"' and weekly='"+weekly+"'"  ;
					}else{
