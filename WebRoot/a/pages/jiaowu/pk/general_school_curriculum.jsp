	    ...
	    <form class="layui-form" action="">
	    	<div class="layui-form-item">
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				      	<select lay-verType="tips" id="xueqi">
				      		<%
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				          ...
				          %>	
				          		<option value="<%=xiaoquSet.getString("id")%>"><%=xiaoquSet.getString("campus_name") %></option>
				          <%}if(xiaoquSet!=null){xiaoquSet.close();} %>
				        </select>
				    </div>
			    </div>
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				          ...
				          <option value="0">全部</option>
				          <%
				          		String chengci = "select id,name from jz_culture_level";
				          		ResultSet chengciSet = db.executeQuery(chengci);
				          		while(chengciSet.next()){
				          %>
				          		<option value="<%=chengciSet.getString("id") %>"><%=chengciSet.getString("name") %></option>
				          <%}if(chengciSet!=null){chengciSet.close();} %>
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline" style="width: 150px;">
				      	<select lay-verType="tips" id="ruxue">
				          <option value="0">全部</option>
				          <%
				          		String ruxuesql = "SELECT school_year FROM class_grade GROUP BY school_year ORDER BY school_year DESC";
				          		ResultSet ruxueSet = db.executeQuery(ruxuesql);
				          		while(ruxueSet.next()){
				          %>
				          		<option value="<%=ruxueSet.getString("school_year") %>"><%=ruxueSet.getString("school_year") %></option>
				          <%}if(ruxueSet!=null){ruxueSet.close();} %>
				        </select>
				    </div>
			    </div>
			    ...
			    <div class="layui-inline">
				    ...
				    <input type="button" name="xxx" value="打印" onclick="myPrint(document.getElementById('myprint'))" class="layui-btn">
...
layui.use(['form','layer','jquery'], function(){
	...
	function find(){
		...
		var ruxue = $("#ruxue").val();		//入学年份
		var major = $("#major").val();		//专业
		var department = $("#department").val();	//院系
		var xiaoqu = $("#xiaoqu").val()
		

		var obj_str1 = {"xueqi":xueqi,"ruxue":ruxue,"major":major,"department":department,"xiaoqu":xiaoqu};
		var obj1 = JSON.stringify(obj_str1)
		var ret_str1=PostAjx('../../../../Api/v1/?p=web/kebiao/getAllSchoolTimetable',obj1,'<%=Suid%>','<%=Spc_token%>');
		obj1 = JSON.parse(ret_str1);
		console.log(obj1);		
		if(obj1.success){
			var data = obj1.data;
			...
			var colspannum = data.length+2;
			//$("#my-table1").attr("style","width : "+width+"px");
			//$("#tab2").attr("style","padding: 0;width: "+width+"px");
			//$("#tab2").attr("colspan",colspannum);

			
			for(var i =0 ;i<data.length;i++){
				$("#class_grade_id").append("<td class='shan'>"+obj1.data[i].classname+"</td>");
					...
					for(var m =0 ;m<timetable.length;m++){
						...
						for(var y = 0; y < array.length;y++){
							...
							if(array[y]=="0"){
								...
								str += deatils[j].classroomname+"<br>";
