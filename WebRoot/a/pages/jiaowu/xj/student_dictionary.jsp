...
<!--        <button class="layui-btn layui-btn-small  layui-btn-primary" >删除</button>-->
        <button class="layui-btn layui-btn-small  layui-btn-primary "  onclick="newDic()" >新建数据字典</button>
        	<%
		       	//获取文件后面的对象 数据
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(1) as row from student_dictionary "+search+";";
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="所属字典"     data-sortable="true" data-filter-control="select" data-visible="true" >所属字典</th>
          ...
          <%
		        ...
		        String typegroup_sql= "select * from student_dictionary "+search+" limit  "+(pages-1)*10+",10;";
		        System.out.println(typegroup_sql);
          		ResultSet groups = db.executeQuery(typegroup_sql);
          		int i = 1;
          		while(groups.next()){
           %>
          <tr>
			...
			<td class=""><%=groups.getString("owndic") %></td>
    ...
    function newDic(){
    	...
    	layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['940px', '60%'],
		  content: 'new_sys_dictionary.jsp' 
		});
    }
    ...
    function look(id){
		...
		layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['980px', '100%'],
		  content: 'sys_dictionary_info.jsp?id='+id 
		});
     ...
     function edit(id){
		...
		layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['940px', '60%'],
		  content: 'edit_sys_dictionary.jsp?id='+id 
		});
    } 
  ...
  layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {  
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          url : basePath+"/Api/v1/?p=web/do/doDelSysDictionary",
			          data : { id: id},  
