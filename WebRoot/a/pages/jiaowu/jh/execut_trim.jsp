...
<%	
		...
		if("table".equals(ac)){
		    ...
		    String nz_sql="select eductional_systme_id,school_year from major where id='"+form_datr.getString("major_id")+"';";
			...
			String sql="SELECT "+
							...
							"tea.teaching_plan_class_id                 AS tea_teaching_plan_class_id,    "+
							...
							"tea.adjustment                             AS tea_adjustment,                "+
							"tea.weeks as weeks "+
					   "FROM   "+
							"teaching_plan tea  "+
							"LEFT JOIN teaching_plan_class tea_class    ON tea_class.id=tea.teaching_plan_class_id  "+
			...
			int j=0;
			String teaching_plan_class_id="";
			while(rs.next()){
			
			j++;
			teaching_plan_class_id=rs.getString("tea_teaching_plan_class_id");
				...
				for(int i=1;i<=(nz*2);i++){
					...
					start_semester.append("<td ><input type='text' name='start_semester"+i+"'  class='layui-input' value='"+rs.getString("start_semester"+i)+"'></td>");
				}
				String adjustment_str="";
				if(rs.getInt("tea_adjustment")==1){
					adjustment_str="<span class=\"layui-badge-dot layui-bg-orange\"></span>";
				}
				tbody.append(
					"<tr id='execut"+j+"'>"+
					    ...
					    "<td ><input type='text' name='total_classes'  class='layui-input' value='"+rs.getInt("total_classes")+"'></td>"+/*课程总学时数*/
					    "<td ><input type='text' name='lecture_classes'  class='layui-input' value='"+rs.getInt("lecture_classes")+"'></td>"+/*理论教学时数*/
					    "<td ><input type='text' name='class_in'  class='layui-input' value='"+rs.getInt("class_in")+"'></td>"+/*课内*/
					    "<td ><input type='text' name='extracurricular_practice_hour'  class='layui-input' value='"+rs.getInt("extracurricular_practice_hour")+"'></td>"+/*独立设置*/
					    "<td ><input type='text' name='test_semester'  class='layui-input' value='"+rs.getString("test_semester")+"'></td>"+/*考试学期*/
					    "<td ><input type='text' name='check_semester'  class='layui-input' value='"+rs.getString("check_semester")+"'></td>"+/*考查学期*/
					    start_semester.toString()+/*学期周课时*/
					    "<td ><input type='text' name='weeks'  class='layui-input' value='"+rs.getString("weeks")+"'></td>"+/*开课周次*/
					    "<td ><input type='text' name='credits'  class='layui-input' value='"+rs.getString("credits")+"'></td>"+/*学分*/
					    "<td >"+adjustment_str+"<button class='layui-btn' onclick='modify(\"execut"+j+"\",\""+rs.getString("tea_id")+"\")'>保存</button></td>"+/*操作*/
					"<tr>"
				);
				total_classes=total_classes+rs.getInt("total_classes");
				total_credits=total_credits+rs.getFloat("credits");
			...
			tbody.append(
					"<tr>"+
					    ...
					    "<td class='ztd'>"+total_credits+"</td>"+/*学分*/
					    "<td class='ztd'>*</td>"+/*操作*/
					"<tr>"
				);
			...
			String weekly="select adjustment,semester_weekly1,semester_weekly2,semester_weekly3,semester_weekly4,semester_weekly5,semester_weekly6,semester_weekly7,semester_weekly8,semester_weekly9,semester_weekly10 from teaching_plan_class where id='"+plan_class_id+"' ";
			ResultSet wee_rs= db.executeQuery(weekly);
			int adjustment_class=0;
			if(wee_rs.next()){
				for(int i=1;i<=(nz*2);i++){
					thead_weeks.append("<th class='th'> <input type='text' name='semester_weekly"+i+"'  class='layui-input' value='"+wee_rs.getString("semester_weekly"+i)+"'></th>");
				}
				adjustment_class=wee_rs.getInt("adjustment");
			}if(wee_rs!=null){wee_rs.close();}
			
			
			/*设置表头*/
			StringBuffer thead = new StringBuffer();
			String adjustment_class_str="";
			if(adjustment_class==1){
				adjustment_class_str="<span class=\"layui-badge-dot layui-bg-orange\"></span>";
			}
			thead.append(
						"<tr>                                                	"+
						...
						"  <th class='th' rowspan='4'>学分</th>              "+
						"  <th class='th' rowspan='4'>操作"+adjustment_class_str+"<button class='layui-btn' onclick='modify(\"weeks\",\""+teaching_plan_class_id+"\")'>修改</button></th>                	"+
						"</tr>                                               	"+
						"<tr >                                                	"+
...
<%
	/*执行更新*/
	if("upsql".equals(ac)){
		String sql_str=request.getParameter("str").toString();
...
width:20%;
		...
		<div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">
			...
			<form class="layui-form"  action="">
		        ...
		        <div id="tb" class="form_top layui-form" style="display: flex;">
		            ...
		            <button class="layui-btn" lay-submit lay-filter="statistics" id="select_button">查询</button>
			    </div>
			</form>
		    <!-- 页面表格展示实现区 -->
			<div  style="display:block;clear:both;">
				<!-- test --> 
				<div id="table">
					<table  class="layui-table tb tb-b c-100 c-t-center "  id="tablew">
						<thead id="theads">
  ...
  <script> 
  		...
  		/*修改*/
  		function modify (domid,planid){
  		
    	var td_from = document.getElementById(domid);
    	var td_inputs = td_from.getElementsByTagName('input');
    	
    	
    	var sql_cout="";
    	
    	if(domid=="weeks"){
    	/*更新 teaching_plan_class*/
	    	for(var i = 0; i < td_inputs.length; i++) {
	    		if((td_inputs.length-1)==i){
	    			sql_cout=sql_cout+td_inputs[i].name+"='"+td_inputs[i].value+"'";
	    		}else{
	    			sql_cout=sql_cout+td_inputs[i].name+"='"+td_inputs[i].value+"',"; 
	    		}
		    }
    		sql_cout=" UPDATE  teaching_plan_class  SET  "+sql_cout+",adjustment='1' where id='"+planid+"';"
    	}else{
    	/*更新 teaching_plan */
    		for(var i = 0; i < td_inputs.length; i++) {
	    		if((td_inputs.length-1)==i){
    		...
    		sql_cout=" UPDATE  teaching_plan  SET  "+sql_cout+",adjustment='1' where id='"+planid+"';"
    	
    	}	
        var upstate=PostAjx("?ac=upsql&str="+sql_cout,"","","");
		...
		layui.use(['laypage', 'form', 'laydate', 'table'], function(){
				  ...
				  /*年选择器*/
				  laydate.render({elem: '#schoolYear' ,type: 'year' ,value: '2010'});
				 ...
				 function table_style(){
				   	...
				   	/* $("#tbodys").css("position","absolute");
				   	 $("#tbodys").css("top","65%");
					 $("#theads").css("position","fixed");
					 $("#theads").css("top","30%");
					 $("#theads").css("z-index","1");*/
