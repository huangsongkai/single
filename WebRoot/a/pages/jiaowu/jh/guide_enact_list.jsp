...
//获取文件后面的对象 数据
		...
		//查找字段名称
 		...
 		//查询的字段局部语句
 		String search=" WHERE p.major_id=m.id AND p.dict_departments_id=d.id and add_worker_id= "+Suid;
...
<html>
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
            ...
            <select name="dict_departments_id" id="dict_departments_id" lay-verify="required" lay-search>
            ...
            <select name="school_year"  id="school_year" lay-verify="required" lay-search>
          ...
          <select size="1" name="state_approve_id" id="state_approve_id" style="width: 83; height: 23" lay-search>
...
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>  
		                 ...
		                 <th data-field="审批状态"  data-sortable="true" data-filter-control="select"  data-visible="true">审批状态</th>
		                  ...
		                  <th data-field="送往审核"     data-sortable="true" data-filter-control="select"  data-visible="true">送往签字</th>
		        ...
		        <tr>
		            ...
		            <a href="guide_batch.jsp?class_id=<%=Rs.getString("sysid")%>&major_id=<%=Rs.getString("major_id")%>&school_year=<%=Rs.getString("school_year")%>&dict_departments_id=<%=Rs.getString("dict_departments_id")%>&departments_name=<%=Rs.getString("departments_name")%>&major_name=<%=Rs.getString("major_name")%>">
		            <%}else{%>
		            <a href="guide_enact.jsp?class_id=<%=Rs.getString("sysid")%>&major_id=<%=Rs.getString("major_id")%>&school_year=<%=Rs.getString("school_year")%>&dict_departments_id=<%=Rs.getString("dict_departments_id")%>&departments_name=<%=Rs.getString("departments_name")%>&major_name=<%=Rs.getString("major_name")%>">
		            <%} %>
		           ...
		           <%}else if("7".equals(stateid)){%>
		            <td>签字成功</td>
		            <td><button class="layui-btn" onclick="showQz('<%=Rs.getString("sysid")%>')">签字意见</button></td> 
		           <%}else if("6".equals(stateid)){%>
		           <td><button class="layui-btn" onclick="sent('<%=Rs.getString("sysid")%>')">可直接提交</button></td>
		           <td><button class="layui-btn" onclick="showQz('<%=Rs.getString("sysid")%>')">签字意见</button></td>
		           <%}else{%>
		           <td><button class="layui-btn" onclick="sentTo('<%=Rs.getString("sysid")%>')">送往签字</button></td>
<%--		           <td><button class="layui-btn" onclick="showQz('<%=Rs.getString("sysid")%>')">签字意见</button></td>--%>
					<td>待送往签字</td>
		           <%}%>
	    ...
	    <script type="text/javascript" >  
				...
				//送往签字
				function sentTo(sysid){
					 window.location.href="?ac=sent_to&sysid="+sysid;
				}
				//直接提交
				function sent(sysid){
					 window.location.href="?ac=sent&sysid="+sysid;
				}
                //新增计划
			     function add(){
			    	 layer.open({
			     ...
			     function showQz(sysid){
			    	 layer.open({
			    		 type: 2,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'guide_audit_opinion.jsp?sysid='+sysid 
			    		});
			     }
	...
	<%if("sent_to".equals(ac)) {
		String sysid=request.getParameter("sysid");
		 if(db.executeUpdate("UPDATE  `teaching_plan_class_guidance` SET `state_approve_id`=5 WHERE `id`='"+sysid+"';")==true){
			   out.print("<script> alert('已发往签字');window.location.replace('./guide_enact_list.jsp') </script>");
			   }else{  out.println("<script> alert('发往签字失败'); </script>"); }  
	}%>    
	<%if("sent".equals(ac)) {
		String sysid=request.getParameter("sysid");
		 if(db.executeUpdate("UPDATE  `teaching_plan_class_guidance` SET `state_approve_id`=7 WHERE `id`='"+sysid+"';")==true){
			   out.print("<script> alert('已提交');window.location.replace('./guide_enact_list.jsp') </script>");
			   }else{  out.println("<script> alert('提交失败'); </script>"); }  
	}%>    
