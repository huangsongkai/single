...
<%	
		...
		if("table".equals(ac)){
			...
			if(form_datr.getString("course_name")!=null && form_datr.getString("course_name").length()>0){
				sql_where.append(" AND dic_cour.course_name='"+form_datr.getString("course_name")+"'   ");
			}
			if(form_datr.getString("course_code")!=null && form_datr.getString("course_code").length()>0){
				sql_where.append(" AND dic_cour.course_code='"+form_datr.getString("course_code")+"'   ");
			}
			if(form_datr.getString("course_nature")!=null && form_datr.getString("course_nature").length()>0){/*课程类别*/
				sql_where.append(" AND dic_nature.id='"+form_datr.get("course_nature")+"'   ");
			}
			if(form_datr.getString("course_category")!=null && form_datr.getString("course_category").length()>0){/*课程性质*/
				sql_where.append(" AND dic.id='"+form_datr.get("course_category")+"'   ");
			}
			if(form_datr.getString("start_semester")!=null && form_datr.getString("start_semester").length()>0){/*学期数*/
				sql_where.append("  AND (start_semester"+form_datr.getString("start_semester")+" <> '' OR start_semester1 <> NULL)   "); 
			}
			/*排序*/
			if(form_datr.getString("start_semester")!=null && form_datr.getString("start_semester").length()>0){/*按照学期数*/
				sql_where.append("  ORDER BY tea.start_semester1 DESC   "); 
			}else
			if(form_datr.getString("start_semester")!=null && form_datr.getString("start_semester").length()>0){/*按照开课类别   必修 选修 。。。*/
				sql_where.append("  ORDER BY tea.course_category_id DESC   "); 
			}else
			if(form_datr.getString("start_semester")!=null && form_datr.getString("start_semester").length()>0){/*按照开课性质*/
				sql_where.append("  ORDER BY tea.course_nature_id DESC   ");  
			}
			//course_category
		    ...
		    //String nz_sql="select eductional_systme_id,school_year from major where id='15';";
			...
			/*计划总条数*/
			String plan_num_sql="select count(1) row  from teaching_plan_guidance tea  "+
														"LEFT JOIN teaching_plan_class_guidance tea_class ON tea_class.id=tea.class_guidance_id "+
			...
			String where_sql="tea_class.dict_departments_id='"+form_datr.get("dict_departments_id")+"' AND  tea_class.major_id='"+form_datr.get("major_id")+"' AND tea_class.school_year='"+form_datr.get("schoolYear")+"'  AND tea_class.state_approve_id='7' "+ sql_where.toString() +" limit "+(form_datr.getInt("page")-1)*form_datr.getInt("limit")+","+form_datr.getInt("limit")+"  ";
			
			String sql="SELECT "+
						"tea_class.id, "+
						"dic.category, "+
						"dic_nature.nature, "+
						"dic_cour.course_name, "+
						"dic_cour.course_code, "+
						"tea.total_classes, "+
						"tea.lecture_classes, "+
						"tea.class_in, "+
						"tea.extracurricular_practice_hour, "+
						"tea.test_semester, "+
						"tea.check_semester, "+
						"tea.start_semester1, "+
						"tea.start_semester2, "+
						...
						"tea.start_semester8, "+
						"tea.start_semester9, "+
						"tea.start_semester10, "+
						"tea.weeks "+
					"FROM   "+
						"teaching_plan_guidance tea   "+
						"LEFT JOIN teaching_plan_class_guidance tea_class ON tea_class.id=tea.class_guidance_id  "+
						"LEFT JOIN dict_course_category dic ON dic.id=tea.course_category_id  "+
						"LEFT JOIN dict_course_nature dic_nature ON dic_nature.id=tea.course_nature_id  "+
						"LEFT JOIN dict_courses dic_cour ON dic_cour.id=tea.course_id  "+
			...
			while(rs.next()){
			    plan_class_id=rs.getString("tea_class.id");
				...
				for(int i=1;i<=(nz*2);i++){
					...
					start_semester_json.put(""+i+"",(temporary+rs.getInt("tea.start_semester"+i)));
					start_semester.append("<td >"+rs.getString("tea.start_semester"+i)+"</td>");
				}
				tbody.append(
					"<tr>"+
						"<td >"+rs.getString("dic_nature.nature")+"</td>"+/*课程类别*/
						"<td >"+rs.getString("dic.category")+"</td>"+/*课程性质*/
					    "<td >"+rs.getString("dic_cour.course_name")+"</td>"+/*课程名称*/
					    "<td >"+rs.getString("dic_cour.course_code")+"</td>"+/*课程编号*/
					    "<td >"+rs.getInt("tea.total_classes")+"</td>"+/*课程总学时数*/
					    "<td >"+rs.getInt("tea.lecture_classes")+"</td>"+/*理论教学时数*/
					    "<td >"+rs.getInt("tea.class_in")+"</td>"+/*课内*/
					    "<td >"+rs.getInt("tea.extracurricular_practice_hour")+"</td>"+/*独立设置*/
					    "<td >"+rs.getString("tea.test_semester")+"</td>"+/*考试学期*/
					    "<td >"+rs.getString("tea.check_semester")+"</td>"+/*考查学期*/
					    start_semester.toString()+/*学期周课时*/
					    "<td >"+rs.getString("tea.weeks")+"</td>"+/*开课周次*/
					    "<td >*</td>"+/*操作*/
					"<tr>"
				);
				total_classes=total_classes+rs.getInt("tea.total_classes");
				lecture_classes=lecture_classes+rs.getInt("tea.lecture_classes");
				class_in=class_in+rs.getInt("tea.class_in");
				extracurricular_practice_hour=extracurricular_practice_hour+rs.getInt("tea.extracurricular_practice_hour");
			}if(rs!=null){rs.close();}
			...
			String weekly="select semester_weekly1,semester_weekly2,semester_weekly3,semester_weekly4,semester_weekly5,semester_weekly6,semester_weekly7,semester_weekly8,semester_weekly9,semester_weekly10 from teaching_plan_class_guidance where id='"+plan_class_id+"' ";
