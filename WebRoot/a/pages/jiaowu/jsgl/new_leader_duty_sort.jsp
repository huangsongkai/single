...
<html>
			...
			<form class="layui-form" action="?ac=post" method="post">
				...
				<div class="layui-form-item inline">
					...
					<div class="layui-input-inline">
						<input type="text" name="dutiesname" lay-verify="required" autocomplete="off" class="layui-input"  value="">
					</div>
				</div>
				<div class="layui-form-item inline" >
					<label class="layui-form-label" >职务编码</label>
					<div class="layui-input-inline">
						<input type="text" name="dutiescode" lay-verify="required" autocomplete="off" class="layui-input" value=""  >
					</div>
				</div>
	...
	if(ac.equals("post")){
		String dutiescode = "";
		String dutiesname = "";
		dutiescode =  request.getParameter("dutiescode");
		dutiesname = request.getParameter("dutiesname");
		String checkSql ="select count(id) as row from teacher_duties t  where t.dutiescode='"+dutiescode+"'";
		int rows = db.Row(checkSql);
		if(rows>0){
			out.print("layer.msg('编码已存在,请勿重新添加')");
		}else{
		String sql = "INSERT INTO teacher_duties (dutiescode,dutiesname) VALUES ('"+dutiescode+"','"+dutiesname+"')";
