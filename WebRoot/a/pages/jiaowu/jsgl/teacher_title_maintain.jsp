...
<!--        <button class="layui-btn " >删除</button>-->
        <button class="layui-btn "  onclick="newDistrbutor()" >新建教师职称</button>
        	<%
		       	//获取文件后面的对象 数据
				...
				//查询的字段局部语句
		 		...
		 		if(search_val.length()>=1){
		 			search="where teacher_title_name like '%"+search_val+"%'";
		 		}else{
		 			search = "where 1=1";
		 		}
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from teacher_title t   "+search;
		        ...
		        String bank_sql= "select t.*  from teacher_title  t "+search+"  limit "+(pages-1)*limits+","+limits+";";
		        ...
		        ArrayList<String> list  = new ArrayList<String>();
            		...
            		while(groups.next()){
            			html_str = "<tr>"
   							+"<td class=\"\">"+groups.getString("teacher_title_name") +"</td>          "
   							+"<td class=\"\">"+groups.getString("teacher_title_grade")+"</td>         "
   							+"<td class=\"\">"+groups.getString("teacher_title_code")+"</td>         "
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="档次"     data-sortable="true" data-filter-control="select" data-visible="true" >档次</th>
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'new_teacher_title_maintain.jsp'
		});
    }
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'edit_teacher_title_maintain.jsp?id='+id 
		});
    } 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          ...
			          data : { id: id,table:"teacher_title"},  
