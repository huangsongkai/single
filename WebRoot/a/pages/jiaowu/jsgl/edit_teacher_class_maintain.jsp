...
<html>
	...
	<body style='height:auto'>
		...
		<div style="margin: 15px;">
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				<legend>编辑教师类别</legend>
			</fieldset>

			<form class="layui-form" action="?ac=post" method="post">
				<%
			        		String id = request.getParameter("id");
			          		String bank_sql = "select t.* from teacher_class t  where  t.id = "+id;
			          		ResultSet banks = db.executeQuery(bank_sql);
			          		int i = 1;
			          		while(banks.next()){
			         %>
				<div class="layui-form-item inline" >
					<label class="layui-form-label">教师类别名称</label>
					<input type="hidden" name="id" value="<%=banks.getString("id") %>">
					<div class="layui-input-inline">
						<input type="text" name="categoryname" lay-verify="required" autocomplete="off" class="layui-input"  value="<%=banks.getString("categoryname") %>">
					</div>
				</div>
				<div class="layui-form-item inline">
					<label class="layui-form-label" >教师类别编码</label>
					<div class="layui-input-inline">
						<input type="text" name="categorycode" lay-verify="required" autocomplete="off" class="layui-input" value="<%=banks.getString("categorycode") %>" readonly placeholder="">
					</div>
				</div>
	...
	if(ac.equals("post")){
		...
		String sql = "update teacher_class t set categoryname='"+categoryname+ "',categorycode='"+categorycode+"'"+" where t.id = "+id;
