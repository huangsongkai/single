...
<html>
			...
			<form class="layui-form" action="?ac=post" method="post">
				...
				<div class="layui-form-item inline">
					...
					<div class="layui-input-inline">
						<input type="text" name="categoryname" lay-verify="required" autocomplete="off" class="layui-input"  value="">
					</div>
				</div>
				<div class="layui-form-item inline" >
					<label class="layui-form-label" >教师类别编码</label>
					<div class="layui-input-inline">
						<input type="text" name="categorycode" lay-verify="required" autocomplete="off" class="layui-input" value=""  >
					</div>
				</div>
	...
	if(ac.equals("post")){
		...
		String checkSql ="select count(id) as row from teacher_class t  where t.categorycode='"+categorycode+"'";
		int rows = db.Row(checkSql);
		if(rows>0){
			out.print("layer.msg('编码已存在,请勿重新添加')");
		}else{
		String sql = "INSERT INTO teacher_class (categorycode,categoryname) VALUES ('"+categorycode+"','"+categoryname+"')";
