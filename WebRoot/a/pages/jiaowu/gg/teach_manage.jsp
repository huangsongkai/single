...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (teaching_research_code like '%"+search_val+"%' or teaching_research_name like '%"+search_val+"%') ";
 		}     	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from teaching_research "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from teaching_research  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
			...
			"<td ><strong>"+Rs.getString("teaching_research_code")+"</strong></td>\r\n"+//教研室编号
			"<td ><strong>"+Rs.getString("teaching_research_name")+"</strong></td>\r\n"+//教研室名称			
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
					             "<button class=\"layui-btn\" onclick=\"edit_teach_manage('"+Rs.getString("id")+"')\">编辑</button>"+
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="教研室编号"  data-sortable="true" data-filter-control="select"  data-visible="true">教研室编号</th>
		                <th data-field="教研室名称"  data-sortable="true" data-filter-control="select"  data-visible="true">教研室名称</th>
		        ...
		        function deletet(id){
				 ...
				 function edit_teach_manage(id){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_teach_manage.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM teaching_research WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除教研室成功');window.location.replace('./teach_manage.jsp');</script>");
	   }
	   else{
		   out.println("<script>parent.layer.msg('删除教研室失败');</script>");
	   }
	 }catch (Exception e){
	    out.println("<script>parent.layer.msg('删除教研室失败');</script>");
	    return;
