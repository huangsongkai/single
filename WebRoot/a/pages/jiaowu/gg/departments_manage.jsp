...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (departments_name like '%"+search_val+"%' or departments_number like '%"+search_val+"%' or departments_english like '%"+search_val+"%') ";
 		}
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_departments "+search+";";
		int zpag= db.Row(zpag_sql);			

		//SQL语句
       	String sql="select * from dict_departments  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
        	String str="<td></td>";
        	if("0".equals(Rs.getString("teach_tag"))){
        		 str="<td>无关</td>";
			}else{
				 str="<td>有关</td>";
			}
        	
        	String iscommon = Rs.getString("iscommon");
        	String iscommon_str = "";
        	if("1".equals(iscommon)){
         	...
         	"<td >"+Rs.getString("departments_number") +"</td>\r\n"+//院系编号
			"<td ><strong>"+Rs.getString("departments_name") +"</strong></td>\r\n"+//院系名称
			"<td ><strong>"+Rs.getString("departments_english")+"</strong></td>\r\n"+//院系英文名称
			"<td ><strong>"+iscommon_str+"</strong></td>\r\n"+//是否常用
			             str//是否与院系有关
						+"<td >"+
...
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                <th data-field="所属分校"  data-sortable="true" data-filter-control="select"  data-visible="true">所属分校</th>
		                <th data-field="院系编号"  data-sortable="true" data-filter-control="select"  data-visible="true">院系编号</th>
		                <th data-field="院系名称"  data-sortable="true" data-filter-control="select"  data-visible="true">院系名称</th>
		                <th data-field="院系英文名称"  data-sortable="true" data-filter-control="select"  data-visible="true">院系英文名称</th>
		                <th data-field="常用标记"  data-sortable="true" data-filter-control="select"  data-visible="true">常用标记</th>
		              	<th data-field="教学标记"  data-sortable="true" data-filter-control="select"  data-visible="true">教学标记</th>
		        ...
		        function ac_tion() {
				 ...
				 function new_departments_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_departments_manage.jsp' 
			    		});
			     }
				 function edit_departments_manage(id){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_departments_manage.jsp?id='+id 
			    		});
			     }
	    </script>
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_departments WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除院系成功');window.location.replace('./departments_manage.jsp');</script>");
	   }
	   else{
		   out.println("<script>parent.layer.msg('删除院系失败');</script>");
	   }
	 }catch (Exception e){
	    out.println("<script>parent.layer.msg('删除院系失败');</script>");
	    return;
