...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (teaching_number like '%"+search_val+"%' or teaching_area_name like '%"+search_val+"%') ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from teaching_area "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from teaching_area  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         	...
         	*/
         	//是否专用判断
         	tagZy=Rs.getString("reserved");
         	if("0".equals(tagZy)){tagZy="";}else{tagZy="&#x1005;";}
         	//体育场地标志
         	tagTycdbz=Rs.getString("stadium");
         	if("0".equals(tagTycdbz)){tagTycdbz="";}else{tagTycdbz="&#x1005;";}
         	html_str="<tr>\r\n"+
         	"<td ><strong>"+Rs.getString("teaching_number")+"</strong></td>\r\n"+//编号
			"<td ><strong>"+Rs.getString("teaching_area_name")+"</strong></td>\r\n"+//教学功能区名称		
			"<td ><i class=\"layui-icon\">"+tagZy+"</td>\r\n"+//是否专用
			"<td ><i class=\"layui-icon\">"+tagTycdbz+"</td>\r\n"+//体育场地标志
			"<td ><strong>"+common.idToFieidName("dict_campus","campus_name",Rs.getString("campus_id"))+"</strong></td>\r\n"+//所在分校		
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
					             "<button class=\"layui-btn\" onclick=\"look('"+Rs.getString("id")+"'@#@'"+Rs.getString("teaching_area_name")+"')\">编辑</button>"+
		        ...
		        <thead>
		            <tr>
		                ...
		                <th data-field="是否专用"  data-sortable="true" data-filter-control="select"  data-visible="true">是否专用</th>
	              	    <th data-field="体育场地标志"  data-sortable="true" data-filter-control="select"  data-visible="true">体育场地标志</th>
		        ...
		        function look(id,name){
		           ...
		           layer.open({
		     		 type: 2,
		     		  ...
		     		  shade: 0.5,
		     		  area: ['100%', '100%'],
		     		  content: 'edit_teaching_function_manage.jsp?id='+id 
		     		});
			     ...
			     function newup_class(){
			    	 layer.open({
			    		 type: 2,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '60%'],
			    		  content: 'new_teaching_function_manage.jsp' 
			    		});
			     }
...
if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM teaching_area WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./teaching_function_manage.jsp');</script>");
	   }
	   else{
