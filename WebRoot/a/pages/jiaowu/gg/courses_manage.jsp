...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (course_name like '%"+search_val+"%' or course_code like '%"+search_val+"%' or tag_name like '%"+search_val+"%') ";
 		}     	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_courses "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_courses  "+search+" order by id desc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         	...
         	*/
         	tagSy=Rs.getString("is_test");
         	tagSj=Rs.getString("is_computer");
         	//判断输出符号
         	if("0".equals(tagSy)){ tagSy=""; }else{tagSy="&#x1005;";}
        	if("0".equals(tagSj)){ tagSj=""; }else{tagSj="&#x1005;";}
         	html_str="<tr>\r\n"+
         	"<td ><strong>"+common.idToFieidName("dict_departments","departments_name",common.idToFieidName("teaching_research","departments_id",Rs.getString("teaching_research_id"))) +"</strong></td>\r\n"+//开课院系
			"<td ><strong>"+common.idToFieidName("teaching_research","teaching_research_name",Rs.getString("teaching_research_id")) +"</strong></td>\r\n"+//开课教研室
			"<td ><strong>"+Rs.getString("course_code")+"</strong></td>\r\n"+//课程编码		
			"<td ><strong>"+Rs.getString("course_name")+"</strong></td>\r\n"+//课程名称
			"<td ><strong>"+common.idToFieidName("dict_courses_class_big","courses_bigclass_name",Rs.getString("dict_courses_class_big_id")) +"</strong></td>\r\n"+//课程大类
			//"<td ><strong>"+Rs.getString("course_system_name")+"</strong></td>\r\n"+//课程体系
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
						     "<button class=\"layui-btn\" onclick=\"edit_courses_manage('"+Rs.getString("id")+"')\">修改</button>"+
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="display: flex;">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary" onclick="new_courses_manage()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe61f;</i> 增加</button>
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
		   ...
		   <div id="asc" class="form_top layui-form" style="display: flex;    float: right;">
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                <th data-field="开课院系"  data-sortable="true" data-filter-control="select"  data-visible="true">开课院系</th>
		                <th data-field="开课教研室"  data-sortable="true" data-filter-control="select"  data-visible="true">开课教研室</th>
		                <th data-field="课程编码"  data-sortable="true" data-filter-control="select"  data-visible="true">课程编码</th>
	              	
		              	<th data-field="课程名称"  data-sortable="true" data-filter-control="select"  data-visible="true">课程名称</th>
		              	<th data-field="课程大类"  data-sortable="true" data-filter-control="select"  data-visible="true">课程大类</th>	              	
<%--		                <th data-field="课程体系"  data-sortable="true" data-filter-control="select"  data-visible="true">课程体系</th>--%>
<%--		              	<th data-field="带实验"  data-sortable="true" data-filter-control="select"  data-visible="true">带实验</th>--%>
<%--		              	<th data-field="带上机"  data-sortable="true" data-filter-control="select"  data-visible="true">带上机</th>--%>
<%--		              	<th data-field="课程标记"  data-sortable="true" data-filter-control="select"  data-visible="true">课程标记</th>--%>
<%--		              	<th data-field="课程符号"  data-sortable="true" data-filter-control="select"  data-visible="true">符号</th>--%>
	    ...
	    <script type="text/javascript">  
		        ...
		        function new_courses_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_courses_manage.jsp' 
			    		});
			     }
			     function edit_courses_manage(id){
			    	 layer.open({
			    		  type: 2,
			    		  title: '编辑课程',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_courses_manage.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_courses WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./courses_manage.jsp');</script>");
	   }
	   else{
