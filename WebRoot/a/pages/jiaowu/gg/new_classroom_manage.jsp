...
<html>
	...
	<body>
		<div style="margin: 15px;">
			...
			<form class="layui-form" action="new_classroom_manage.jsp?ac=add" method="post" >
              <div class="layui-form-item inline" style="width: 40%">
						...
						<select name="building_id" class="layui-input" lay-filter="building_id" lay-verify="required">
                         ...
                         //查询状态
                          ResultSet campusRs = db.executeQuery("SELECT  id,building_name FROM teaching_building;");
                            while(campusRs.next()){
                            %>
                          <option value="<%=campusRs.getString("id")%>"><%=campusRs.getString("building_name") %></option>
						...
						<select name="teaching_area_id" class="layui-input" lay-filter="teaching_area_id" lay-verify="required">
                         ...
                         //查询状态
                          ResultSet gnRs = db.executeQuery("SELECT  id,teaching_area_name FROM teaching_area;");
                            while(gnRs.next()){
                            %>
                          <option value="<%=gnRs.getString("id")%>"><%=gnRs.getString("teaching_area_name") %></option>
                ...
                <div class="layui-form-item inline" style=" width: 40%">
					...
					<div class="layui-input-inline">
						<input type="text" id="classroom_number" name="classroom_number" class="layui-input" lay-verify="required" >
					</div>
				</div>	
				<div class="layui-form-item inline" style=" width: 40%">
					<label class="layui-form-label" style=" width: 30%">教室名称</label>
					<div class="layui-input-inline">
						<input type="text" id="classroom_name" name="classroomname" class="layui-input" lay-verify="required" >
					</div>
				</div>	
				<div class="layui-form-item inline" style=" width: 40%">
					<label class="layui-form-label" style=" width: 30%">上课容纳人数</label>
					<div class="layui-input-inline">
						<input type="text" id="goclass_number" name="goclass_number" class="layui-input"  lay-verify="required">
					</div>
				</div>		
				<div class="layui-form-item inline" style=" width: 40%">
					<label class="layui-form-label" style=" width: 30%">考试容纳人数</label>
					<div class="layui-input-inline">
						<input type="text" id="exam_number" name="exam_number" class="layui-input"  lay-verify="required">
					</div>
				</div>				
...
<% if("add".equals(ac)){ 
	...
	try{
	   String sql="INSERT INTO `classroom` (`classroom_number`,`classroomname`,`dedicated`,`teaching_area_id`,`building_id`,`goclass_number`,`exam_number`,`state`) VALUES ('"+classroom_number+"','"+classroomname+"','"+dedicated+"','"+teaching_area_id+"','"+building_id+"','"+goclass_number+"','"+exam_number+"','"+state+"');";
