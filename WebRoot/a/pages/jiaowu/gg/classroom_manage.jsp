...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (classroom_number like '%"+search_val+"%' or classroomname like '%"+search_val+"%') ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from classroom "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from classroom  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         	...
         	*/
         	//是否专用判断
         	tagZy=Rs.getString("dedicated");
         	if("0".equals(tagZy)){tagZy="";}else{tagZy="&#x1005;";}
         	//状态
         	tagZt=Rs.getString("state");
         	if("0".equals(tagZt)){tagZt="";}else{tagZt="&#x1005;";}
         	html_str="<tr>\r\n"+
         	"<td ><strong>"+Rs.getString("classroom_number")+"</strong></td>\r\n"+//教室编号
			"<td ><strong>"+Rs.getString("classroomname")+"</strong></td>\r\n"+//教室名称	
			"<td ><i class=\"layui-icon\">"+tagZy+"</td>\r\n"+//是否专用
			"<td ><strong>"+common.idToFieidName("teaching_area","teaching_area_name",Rs.getString("teaching_area_id"))+"</strong></td>\r\n"+//教学功能区名称
			"<td ><strong>"+common.idToFieidName("teaching_building","building_name",Rs.getString("building_id"))+"</strong></td>\r\n"+//教学楼名称		
			"<td ><strong>"+Rs.getString("goclass_number")+"</strong></td>\r\n"+//上课容纳人数
			"<td ><strong>"+Rs.getString("exam_number")+"</strong></td>\r\n"+//考试容纳人数
			"<td ><i class=\"layui-icon\">"+tagZt+"</td>\r\n"+//状态
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
						    "<button class=\"layui-btn\" onclick=\"edit_classroom_manage('"+Rs.getString("id")+"')\">修改</button>"+
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="display: flex;">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary" onclick="new_classroom_manage()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe61f;</i> 增加</button>
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                <th data-field="教室编号"  data-sortable="true" data-filter-control="select"  data-visible="true">教室编号</th>
		                <th data-field="教室名称"  data-sortable="true" data-filter-control="select"  data-visible="true">教室名称</th>
		                <th data-field="是否专用"  data-sortable="true" data-filter-control="select"  data-visible="true">是否专用</th>
	              	    <th data-field="教学功能区名称"  data-sortable="true" data-filter-control="select"  data-visible="true">教学功能区名称</th>
	              	    <th data-field="教学楼名称"  data-sortable="true" data-filter-control="select"  data-visible="true">教学楼名称</th>
		                <th data-field="上课容纳人数"  data-sortable="true" data-filter-control="select"  data-visible="true">上课容纳人数</th>
		                <th data-field="考试容纳人数"  data-sortable="true" data-filter-control="select"  data-visible="true">考试容纳人数</th>
	    ...
	    <script type="text/javascript">  
		        ...
		        function new_classroom_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_classroom_manage.jsp' 
			    		});
			     }
			     function edit_classroom_manage(id){
			    	 layer.open({
			    		  type: 2,
			    		  title: '编辑教室',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_classroom_manage.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM classroom WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./classroom_manage.jsp');</script>");
	   }
	   else{
