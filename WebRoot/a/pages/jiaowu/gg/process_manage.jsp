...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"			and 	(course_name like '%"+search_val+"%')";
 		}
       	/**
       	 *分页相关
       	 */
		...
		//SQL语句
       	String sql="SELECT id,course_name,process_symbol FROM  dict_courses   "+search+" order by id desc limit  "+(pages-1)*limits+","+limits+";";
       
       	System.out.println("进程页面==="+sql);
        ...
        while(Rs.next()){
			...
			"<td ><i class=\"layui-icon\">&#xe60a;</i><strong>"+common.idToFieidName("dict_process_symbol","process_symbol",Rs.getString("process_symbol")) +"</strong></td>\r\n"+//进程符号
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
					             "<button class=\"layui-btn\" onclick=\"edit_process_manage('"+Rs.getString("id")+"')\">修改</button>"+
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="display: flex;">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary" onclick="new_process_manage()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe61f;</i> 增加</button>
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                <th data-field="进程名称"  data-sortable="true" data-filter-control="select"  data-visible="true">进程名称</th>
		                <th data-field="进程符号"  data-sortable="true" data-filter-control="select"  data-visible="true">进程符号</th>              	
		        ...
		        function ac_tion() {
			     ...
			     function new_process_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_process_manage.jsp' 
			    		});
			     }
			     function edit_process_manage(id){
			    	 layer.open({
			    		  type: 2,
			    		  title: '编辑进程',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_process_manage.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	...
	try{
	   ...
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除进程成功');window.location.replace('./process_manage.jsp');</script>");
	   }
	   else{
		   out.println("<script>parent.layer.msg('删除进程失败');</script>");
	   }
	 }catch (Exception e){
	    out.println("<script>parent.layer.msg('删除进程失败');</script>");
	    return;
