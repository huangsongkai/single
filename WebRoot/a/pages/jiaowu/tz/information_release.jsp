  ...
  <div class="content">
	...
	<div id="tb" class="form_top layui-form" style="">
       <%--<input type="text" class="layui-input textbox-text" placeholder="" style="" id="search">
        ...
        <button class="layui-btn layui-btn-small  layui-btn-primary "  onclick="newDistrbutor()" >新建信息</button>
        	<%
		       	//获取文件后面的对象 数据
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
             <tr>
              ...
              <th data-field="发布时间"  data-sortable="true" data-filter-control="select" data-visible="true">发布时间</th>
 	 ...
 	 $.ajax( {
 		...
 		success : function(data) { 		   
            ...
            document.getElementById("tj").innerHTML+=`<tr>     
 					...
 					<td class=""><a onclick="look(`+jsonData.threads[i].id+`)">预览</a><a onclick="editDistrbutor(`+jsonData.threads[i].id+`)">编辑</a><a onclick="deletet(`+jsonData.threads[i].id+`)">删除</a></td>
 			      </tr>`
              } 
    ...
    function lookDistrbutor(id){
		layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['980px', '100%'],
		  content: 'detail_distrbutor_copy.jsp?id='+id 
		});
    }
    function look(id){
  ...
  layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) { 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          url : basePath+"/Api/v1/?p=web/cms/articles/delArticles",
			          data : JSON.stringify(strvalue),  
			          success : function(data){  
			          var datas = eval('(' + data + ')');
