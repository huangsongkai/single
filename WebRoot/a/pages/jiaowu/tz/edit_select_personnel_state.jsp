		   ...
		   <div class="lb">
			<select id="s1" name=Jsrbh class="myselect" style="width: 162; height: 260"  ondblclick="javascript:addbj();" size="10"  multiple>                      
             ...
             <div class="center">      
              ...
              <input type="button" value="<<" name="removeall" onclick ="javascript:removebjall();" class="button"></p>  
             </div>
            <div class="lb">
...
//回显已选人员
 ...
 $.ajax( {
		...
		type : "POST",
		// 提交的网址
		url : basePath+"/Api/v1/?p=web/users/selectUserBycmsId",
		// 提交的数据
		...
		success : function(data) {	
 	        ...
 	        if(jsonData.success){ 
 	         var inner;   
	       for(var i=0;i<jsonData.auditperson.length;i++){
              inner+=`<option value ="`+jsonData.auditperson[i].uid+`">`+jsonData.auditperson[i].username+`</option>`;
		       }
	        $("#s2").html(inner);}
     },
		 headers : {
	    ...
	    function cz(){
               ...
               parent.$('#auditPerson').val(strs);
               parent.$('#auditperson').val(str);
