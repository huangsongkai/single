...
while(tpcRs.next()){
	int zkcsRs=db.Row("SELECT  count(*) as row FROM  teaching_task WHERE teaching_task_class_id="+tpcRs.getString("id")+";");
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
            ...
            %>
             ...
             <%}if(school_yearRs!=null){school_yearRs.close();} %>
         		...
         		<button class="layui-btn layui-btn-small  layui-btn-primary" onclick="quanbu()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe615;</i> 全部班级</button>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>  
		                 ...
		                 <th data-field="设置"  data-sortable="true" data-filter-control="select"  data-visible="true">设置</th>                  
		       ...
		       //求学期学号
		        ...
		        <tr>
		            ...
		            <td><%=year%></td>
		             <td><a href="teach_all_list.jsp?class_id=<%=Rs.getString("taskid")%>&school_year=<%=years%>&departments_name=<%=Rs.getString("departments_name")%>&years=<%=years %>" title="查看学期信息"><%=years%></a>      </td>
		            <td>
		            <a href="teach_arrange_list.jsp?class_id=<%=Rs.getString("taskid")%>&school_year=<%=years%>&departments_name=<%=Rs.getString("departments_name")%>">
		                                [<%=Rs.getString("majors_number")%>]<%=Rs.getString("major_name")%> <span class="layui-badge"><%=Rs.getString("data_num")%>条</td>	           
		             </a></strong></td>
		             <td><%=Rs.getString("class_name")%></td>
		           <td><a href="teach_arrange_list.jsp?class_id=<%=Rs.getString("taskid")%>&school_year=<%=years%>&departments_name=<%=Rs.getString("departments_name")%>"><strong>教师设置</strong></a></td>
	    ...
	    <script type="text/javascript">  
				...
				function quanbu(){
					...
					window.location.href = "./teach_all_list.jsp";
