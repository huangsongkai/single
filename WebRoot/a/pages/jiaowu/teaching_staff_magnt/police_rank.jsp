        ...
        <button class="layui-btn " onclick="location.reload()" > 刷新</button>
        	<%
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from staff_rank t   "+search;
		        ...
		        String bank_sql= "select t.*,t1.birthday,t1.nation,t1.work_time,t1.education,t1.administrative_level  from staff_rank  t LEFT JOIN teacher_basic t1 ON t.id_number=t1.id_number "+search+"  limit "+(pages-1)*limits+","+limits+";";
				String html_str = "";
	          ...
	          <div id="field">
	           ...
	           <div>
							...
							<input type="text" class="layui-btn" value="" id='wenjianname' readonly placeholder="请上传Excel" style="background: rgb(227, 227, 227); color:black;">
		       				  <a href ="./jingxian.xls" target="_blank" style="font-size:16px;margin-left:15px;">模板下载</a>   
		        </div>
	    </form>   
	    </div>
...
<%--              <th data-field="文化程度" data-sortable="true" data-filter-control="select" data-visible="true" >文化程度</th>--%>
<%--              <th data-field="行政级别" data-sortable="true" data-filter-control="select" data-visible="true" >行政级别</th>--%>
              <th data-field="授衔时间" data-sortable="true" data-filter-control="select" data-visible="true" >授衔时间</th>
              <th data-field="警衔" data-sortable="true" data-filter-control="select" data-visible="true" >警衔</th>
              <th data-field="类别" data-sortable="true" data-filter-control="select" data-visible="true" >类别</th>
              <th data-field="变动情况" data-sortable="true" data-filter-control="select" data-visible="true" >变动情况</th>
<%--              <th data-field="类别" data-sortable="true" data-filter-control="select" data-visible="true" >类别</th>--%>
			...
			$(function() {
		        $("#file_form").submit(
		                function() {
		                    ...
		                    var updaetFiled = '';
		                    var other = "";
		                   	var updaetFiled = 'police_num,id_number,name,type,rank,rank_time,bianhua';
...
if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM staff_rank WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./police_rank.jsp');</script>");
	   }
	   else{
