          ...
          <button class="layui-btn "  id="batchDelr" > 批量审核</button>
        	<%
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from person_change t   "+search;
		        ...
		        String bank_sql= "SELECT t.*,t1.teacher_name,teacher_number  FROM  person_change t left join teacher_basic t1 on t.teacherid=t1.id  "+search+"  limit "+(pages-1)*limits+","+limits+";";
		        String html_str = "";
				StringBuffer sb = new StringBuffer();
            		...
            		while(customerPrs.next()){
            			...
            			html_str = "<tr id='"+customerPrs.getString("id")+"'>"
   							...
   							+"<td class=\"\">"+customerPrs.getString("work_address")+"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("branch") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("work_info") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("work_people") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("start_date") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("end_date") +"</td>          "
   							+"<td class=\"\">"+btn+"</td> "
						+"</tr>"; 
						sb.append(html_str);
           ...
           <thead>
		            <tr>
		             	<th data-field="state"   data-checkbox="true"><input type="checkbox" name="" ></th>
		              	...
		              	<th data-field="工作单位"  data-sortable="true" data-filter-control="select"  data-visible="true">工作单位</th>
		              	<th data-field="部门"  data-sortable="true" data-filter-control="select"  data-visible="true">部门</th>
		              	<th data-field="工作内容"  data-sortable="true" data-filter-control="select"  data-visible="true">工作内容</th>
		              	<th data-field="证明人"  data-sortable="true" data-filter-control="select"  data-visible="true">证明人</th>
		              	<th data-field="开始日期"  data-sortable="true" data-filter-control="select"  data-visible="true">开始日期</th>
		              	<th data-field="结束时间"  data-sortable="true" data-filter-control="select"  data-visible="true">结束时间</th>
...
if("check".equals(ac)){ 
	...
	try{
		...
		for(int i=0;i<id.length;i++){
	   		 dsql="update  person_change  set examine=4 WHERE id='"+id[i]+"';";
	   		delState = db.executeUpdate(dsql);
		}
	   if(delState){
		   out.println("<script>parent.layer.msg('审核成功');window.location.replace('./person_change_check.jsp');</script>");
	   }
	   else{
