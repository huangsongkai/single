          ...
          <button class="layui-btn "  id="batchDelr" > 批量审核</button>
        	<%
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from person_position_change t   "+search;
		        ...
		        String bank_sql= "SELECT t.*,t1.teacher_name,teacher_number  FROM  person_position_change t left join teacher_basic t1 on t.teacherid=t1.id  "+search+"  limit "+(pages-1)*limits+","+limits+";";
				String html_str = "";
				StringBuffer sb = new StringBuffer();
            		...
            		while(customerPrs.next()){
            			...
            			html_str = "<tr id='"+customerPrs.getString("id")+"'>"
   							...
   							+"<td class=\"\">"+customerPrs.getString("postion_name") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("level") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("wen_num") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("wages") +"</td>          "
   							+"<td class=\"\">"+btn+"</td> "
						+"</tr>"; 
						sb.append(html_str);
...
if("check".equals(ac)){ 
	...
	try{
		...
		for(int i=0;i<id.length;i++){
	   		 dsql="update  person_position_change  set examine=4 WHERE id='"+id[i]+"';";
	   		delState = db.executeUpdate(dsql);
		}
	   if(delState){
		   out.println("<script>parent.layer.msg('审核成功');window.location.replace('./position_change_check.jsp');</script>");
	   }
	   else{
