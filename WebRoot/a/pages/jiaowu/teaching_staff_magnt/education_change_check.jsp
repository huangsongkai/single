          ...
          <button class="layui-btn "  id="batchDelr" > 批量审核</button>
        	<%
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from person_education_change t   "+search;
		        ...
		        String bank_sql= "SELECT t.*,t1.teacher_name,teacher_number  FROM  person_education_change t left join teacher_basic t1 on t.teacherid=t1.id  "+search+"  limit "+(pages-1)*limits+","+limits+";";
				String html_str = "";
				StringBuffer sb = new StringBuffer();
            		...
            		while(customerPrs.next()){
            			...
            			html_str = "<tr id='"+customerPrs.getString("id")+"'>"
   							...
   							+"<td class=\"\">"+customerPrs.getString("degree")+"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("degree_num") +"</td>          "
   							+"<td class=\"\">"+customerPrs.getString("degree_date") +"</td>          "
   							+"<td class=\"\">"+btn+"</td> "
						+"</tr>"; 
						sb.append(html_str);
           ...
           <thead>
		            <tr>
		             	<th data-field="state"   data-checkbox="true"><input type="checkbox" name="" ></th>
		              	...
		              	<th data-field="学历证号"  data-sortable="true" data-filter-control="select"  data-visible="true">学历证号</th>
		              	<th data-field="毕业时间"  data-sortable="true" data-filter-control="select"  data-visible="true">毕业时间</th>
...
if("check".equals(ac)){ 
	...
	try{
		...
		for(int i=0;i<id.length;i++){
	   		 dsql="update  person_education_change  set examine=4 WHERE id='"+id[i]+"';";
	   		delState = db.executeUpdate(dsql);
		}
	   if(delState){
		   out.println("<script>parent.layer.msg('审核成功');window.location.replace('./education_change_check.jsp');</script>");
	   }
	   else{
