...
<%
	...
	if(Suserole.equals("1")){
		teacherSql =" select id,teacher_name,teacher_number from teacher_basic where id="+Sassociationid ;
	}else{
		teacherSql = " select id,teacher_name,teacher_number from teacher_basic ";
	}
%>
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline">
				      	<select lay-verType="tips" lay-search lay-filter="teacher" id="teacher" name="teacher">
				      		...
				      		<%
				      			ResultSet teachRs = db.executeQuery(teacherSql);
				      			while(teachRs.next()){
				      				out.print("<option value='"+teachRs.getString("id")+"'>"+teachRs.getString("teacher_number")+"_"+teachRs.getString("teacher_name") +"</option>");
				      			}if(teachRs!=null)teachRs.close();
				      		%>
				        </select>
				    </div>
		      	</div>	
			    <div class="layui-inline">
				    <input type="button" id="excelid" name="xxx" value="导出excel" onclick="method5('my-table1');" class="layui-btn 、">
		      	</div>
			</div>
			<!-- 表格 -->
