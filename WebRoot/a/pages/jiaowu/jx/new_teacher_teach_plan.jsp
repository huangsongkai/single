	    	...
	    	<div class="layui-form-item">
		  		<div class="layui-inline">
				    ...
				    <div class="layui-input-inline">				    	
				    	<select name="teacher_name" lay-verify="required"  lay-filter="teacherChange" lay-search>
					        <option value="0" selected=""></option>
					         <%
					   		      String teacherSql ="";
					         	if(Suserole.equals("1")){//如果是老师查询自己的
					        	 teacherSql = "SELECT DISTINCT(teacherid) id,t2.teacher_name FROM teaching_task_teacher t LEFT JOIN teaching_task t1 ON t.teaching_task_id=t1.id  LEFT JOIN teacher_basic t2 ON t.teacherid=t2.id where  t1.typestate=2 and  t1.semester='"+semester+"'  and t.teacherid="+Sassociationid;
					         	}else{
					        	 teacherSql = "SELECT DISTINCT(teacherid) id,t2.teacher_name FROM teaching_task_teacher t LEFT JOIN teaching_task t1 ON t.teaching_task_id=t1.id  LEFT JOIN teacher_basic t2 ON t.teacherid=t2.id where  t1.typestate=2 and  t1.semester='"+semester+"' ";
					         	}
					        	ResultSet teacherBase = db.executeQuery(teacherSql);
					        	while(teacherBase.next()){
					         %>
					         	<option value="<%=teacherBase.getString("id") %>" ><%=teacherBase.getString("teacher_name") %> 
					         	</option>
					         <%}if(teacherBase!=null){teacherBase.close();}%>
					      </select>
				    </div>
			    </div>
		  		<div class="layui-inline">
...
<%--					        <option value="0" selected=""></option>--%>
<%--					         <%--%>
<%--					        	String classSql = "SELECT	t.*,t4.class_name,t2.course_name  FROM	teaching_task t    	LEFT JOIN dict_courses t2 ON t.course_id=t2.id  LEFT JOIN class_grade t4 ON t.class_id=t4.id	where t.typestate=2 and  t.semester='"+semester+"' ";--%>
...
<%--				    <input type="button" name="xxx" value="查找" class="layui-btn">--%>
			...
			<div class="layui-form-item">
			    ...
			    <div class="layui-inline">
				    ...
				    <div class="layui-input-inline">				    	
				      	<input type="text" name="class_name" value="" class="layui-input" readonly>
				    </div>
			    </div>
			        <div class="layui-inline">
			...
			form.on('select(classChange)',function(data){
    	       ...
    	       var curWwwPath = window.document.location.href;
	    		...
	    		if(obj.success && obj.resultCode=="1000"){
		    		...
		    		$('input[name="class_name"]').val(obj.data[0].class_name);
	    		}else{
	    		}
				})
				
			form.on('select(teacherChange)',function(data){
				var semester = $('input[name="semester"]').val();
				var str = {"semester":semester,"teacherid":data.value};
	    		var obj = JSON.stringify(str);
    	       var curWwwPath = window.document.location.href;
			    var pathName = window.document.location.pathname;
			    var pos = curWwwPath.indexOf(pathName);
			    ...
			    var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1);
			    var basePath=localhostPath+projectName+"/";
			    var url =basePath+"/Api/v1/?p=web/get/teaching_plan1";
	    		var ret_str=PostAjx(url,obj,'<%=Suid%>','<%=Spc_token%>');
	    		var obj = JSON.parse(ret_str);
	    		if(obj.success && obj.resultCode=="1000"){
		    		$('select[name="class_name"]').html(obj.data);
	    		}else{
	    		}
	    		form.render();
				})
