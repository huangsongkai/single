...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (courses_bigclass_code like '%"+search_val+"%' or courses_bigclass_name like '%"+search_val+"%' ) ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_courses_class_big "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_courses_class_big  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         	...
         	*/
         	String show = Rs.getString("show_mark");
         	if(show.equals("1")){show = "是";}else{show="否";}
         		
         	html_str="<tr>\r\n"+
         	"<td ><strong>"+Rs.getString("id")+"</strong></td>\r\n"+//编号
			"<td ><strong>"+Rs.getString("courses_bigclass_code")+"</strong></td>\r\n"+//课程大类代码
			"<td ><strong>"+Rs.getString("courses_bigclass_name")+"</strong></td>\r\n"+//课程大类名称
			"<td ><strong>"+Rs.getString("courses_sort")+"</strong></td>\r\n"+//排列顺序码
		//	"<td ><strong>"+Rs.getString("big_class_tag")+"</strong></td>\r\n"+//课程大类标识-见字典编码bigClassTag
			"<td ><strong>"+show+"</strong></td>\r\n"+//课表是否标记
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
					             "<button class=\"layui-btn\" onclick=\"look('"+Rs.getString("id")+"'@#@'"+Rs.getString("id")+"')\">编辑</button>"+
					             "<button class=\"layui-btn\" onclick=\"delshan('"+Rs.getString("id")+"')\">删除</button>"+
					 	"</div>"+
						"</td>\r\n"+
				      "</tr>\r\n\r\n";
		   list.add(html_str);
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="课程大类代码"  data-sortable="true" data-filter-control="select"  data-visible="true">课程大类代码</th>
		                <th data-field="课程大类名称"  data-sortable="true" data-filter-control="select"  data-visible="true">课程大类名称</th>
		                <th data-field="排列顺序码"  data-sortable="true" data-filter-control="select"  data-visible="true">排列顺序码</th>
<%--		                <th data-field="课程大类标识-见字典编码bigClassTag"  data-sortable="true" data-filter-control="select"  data-visible="true">课程大类标识-见字典编码bigClassTag</th>--%>
	              	  	<th data-field="课表是否标记"  data-sortable="true" data-filter-control="select"  data-visible="true">课表是否标记</th>
	    ...
	    <script type="text/javascript">  
		        ...
		        function look(id,name){
		           ...
		           layer.open({
		     		 type: 2,
		     		  title: '查看【'+name+'】课程大类信息',
		     		  shadeClose: true,
		     		  maxmin:1,
		     		  shade: 0.5,
		     		  area: ['100%', '100%'],
		     		  content: 'edit_class_big.jsp?id='+id 
		     		});
...
if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_courses_class_big WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./class_big.jsp');</script>");
	   }
	   else{
