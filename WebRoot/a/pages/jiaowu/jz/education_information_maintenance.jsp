...
<!--        <button class="layui-btn " >删除</button>-->
        <button class="layui-btn "  onclick="newDistrbutor()" >新建学历信息</button>
        	<%
		       	//获取文件后面的对象 数据
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from jz_educational_info t   "+search;
		        ...
		        String bank_sql= "select t.*  from jz_educational_info  t "+search+"  limit "+(pages-1)*limits+","+limits+";";

				String html_str = "";
		        ArrayList<String> list  = new ArrayList<String>();
            		ResultSet groups = db.executeQuery(bank_sql);
            		while(groups.next()){
            			String statuss = groups.getString("gaoji");
            			if(statuss.equals("1")){
            				statuss = "博士研究生";
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="高基学历"     data-sortable="true" data-filter-control="select" data-visible="true" >高基学历</th>
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'new_education_information_maintenance.jsp'
		});
    }
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'edit_education_information_maintenance.jsp?id='+id 
		});
    } 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          ...
			          data : { id: id,table:"jz_educational_info"},  
