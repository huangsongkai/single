...
<html>
	...
	<body style='height:auto'>
		...
		<div style="margin: 15px;">
			...
			<form class="layui-form" action="?ac=post" method="post">
				...
				<div class="layui-form-item inline">
					<label class="layui-form-label" >学历编码</label>
					<div class="layui-input-inline">
						<input type="text" name="code" lay-verify="required" autocomplete="off" class="layui-input" value="<%=banks.getString("code") %>"  readonly>
					</div>
				</div>
				<div class="layui-form-item inline">
					<label class="layui-form-label" >高基学历</label>
					<div class="layui-input-inline">
						 <select id="gaoji" name="gaoji"  lay-verify="required" >
							 	<%
							 		String statuss = banks.getString("gaoji");
							 		if(statuss.equals("1")){
							 			%>
							 				     <option value="1" selected>博士研究生</option>
										        <option value="2">硕士研究生</option>
										        <option value="3">本科</option>
										        <option value="4">专科及以下</option>
							 			<%
							 		}else if(statuss.equals("2")){%>
							 				  <option value="1" >博士研究生</option>
										        <option value="2" selected>硕士研究生</option>
										        <option value="3">本科</option>
										        <option value="4">专科及以下</option>
							 		<%}else if(statuss.equals("3")){
							 			...
							 			<%
							 				 ...
							 				 <option value="1" >博士研究生</option>
										        <option value="2">硕士研究生</option>
										        <option value="3">本科</option>
										        <option value="4" selected>专科及以下</option>
							 	<%	}else{
							 		out.println("<option value='1'>博士研究生</option><option value='2'>硕士研究生</option><option value='3'>本科</option><option value='4'>专科及以下</option>");
							 	}%>
						      </select>		
					</div>
				</div>
	...
	if(ac.equals("post")){
		...
		gaoji =  request.getParameter("gaoji");
		int ids = Integer.parseInt(request.getParameter("id"));
		String sql = "update jz_educational_info t set name='"+name+  "',code='"+code+  "',gaoji='"+gaoji+"'  where t.id = "+id;
		boolean statuse = db.executeUpdate(sql);
		if(statuse){
