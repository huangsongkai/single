...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (nature like '%"+search_val+"%' or nature_code like '%"+search_val+"%' ) ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_course_nature "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_course_nature  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
			...
			"<td ><strong>"+Rs.getString("nature")+"</strong></td>\r\n"+//性质名称
			"<td ><strong>"+Rs.getString("nature_code")+"</strong></td>\r\n"+//性质编码
					"<td >"+
...
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="性质名称"  data-sortable="true" data-filter-control="select"  data-visible="true">性质名称</th>
		                <th data-field="性质编码"  data-sortable="true" data-filter-control="select"  data-visible="true">性质编码</th>
	    ...
	    <script type="text/javascript">  
		        ...
		        function look(id,name){
		           ...
		           layer.open({
		     		 type: 2,
		     		  title: '查看【'+name+'】课程性质信息',
		     		  shadeClose: true,
		     		  maxmin:1,
		     		  shade: 0.5,
		     		  area: ['100%', '100%'],
		     		  content: 'edit_class_natrue.jsp?id='+id 
		     		});
...
if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_course_nature WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./class_nature.jsp');</script>");
	   }
	   else{
