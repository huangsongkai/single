        ...
        <button class="layui-btn "  onclick="newDistrbutor()" >新建成绩等级</button>
        	<%
				...
				//查询的字段局部语句
		 		...
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from jz_achievement_mark t   "+search;
		        ...
		        String bank_sql= "select t.*  from jz_achievement_mark  t "+search+"  limit "+(pages-1)*limits+","+limits+";";

				String html_str = "";
		        ArrayList<String> list  = new ArrayList<String>();
            		ResultSet groups = db.executeQuery(bank_sql);
            		while(groups.next()){
            			String statuss = groups.getString("status");
            			if(statuss.equals("1")){
            				statuss = "未进行";
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="默认分数"     data-sortable="true" data-filter-control="select" data-visible="true" >默认分数</th>
              <th data-field="考试状态"     data-sortable="true" data-filter-control="select" data-visible="true" >考试状态</th>
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'new_performance_marking_maintenance.jsp'
		});
    }
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'edit_performance_marking_maintenance.jsp?id='+id 
		});
    } 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          ...
			          data : { id: id,table:"jz_achievement_mark"},  
