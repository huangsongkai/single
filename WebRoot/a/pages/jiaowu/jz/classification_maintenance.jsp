...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (category like '%"+search_val+"%' or category_code like '%"+search_val+"%' ) ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_course_category "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_course_category  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
			...
			"<td ><strong>"+Rs.getString("category_code")+"</strong></td>\r\n"+//课程类别编码
					"<td >"+
...
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="性质名称"  data-sortable="true" data-filter-control="select"  data-visible="true">课程类别名</th>
		                <th data-field="性质编码"  data-sortable="true" data-filter-control="select"  data-visible="true">课程类别编码</th>
	    ...
	    <script type="text/javascript">  
		        ...
		        function look(id,name){
		           ...
		           layer.open({
		     		 type: 2,
		     		  title: '查看【'+name+'】课程分类信息',
		     		  maxmin:1,
		     		  shade: 0.5,
		     		 area: ['940px', '60%'],
		     		  content: 'edit_classification_maintenance.jsp?id='+id 
		     		});
			     ...
			     function newup_class(){
			    	 layer.open({
			    		 type: 2,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '60%'],
			    		  content: 'new_classification_maintenance.jsp' 
			    		});
			     }
...
if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_course_category WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./classification_maintenance.jsp');</script>");
	   }
	   else{
