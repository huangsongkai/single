...
<html>
	...
	<body style='height:auto'>
		...
		<div style="margin: 15px;">
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				<legend>编辑成绩标志</legend>
			</fieldset>

			<form class="layui-form" action="?ac=post" method="post">
				<%
			        		String id = request.getParameter("id");
			          		String bank_sql = "select t.* from jz_achievement_mark t  where  t.id = "+id;
			          		ResultSet banks = db.executeQuery(bank_sql);
			          		int i = 1;
			          		while(banks.next()){
			         %>
				<div class="layui-form-item inline" >
					<label class="layui-form-label">成绩标志名称</label>
					...
					<div class="layui-input-inline">
						 <select id="status" name="status"  lay-verify="required" >
							 	<%
							 		...
							 		if(statuss.equals("1")){
							 			%>
							 				     <option value="1" selected>未进行</option>
							 			...
							 			<%
									        ...
									        <option value="2" selected>缓考</option>
									        <option value="3">缺考</option>
									        <option value="4">考试舞弊</option>
							 		<%}else if(statuss.equals("3")){
							 			...
							 			<%
									        ...
									        <option value="2">缓考</option>
									        <option value="3" >缺考</option>
									        <option value="4" selected>考试舞弊</option>
							 	<%	}else{
							 		out.println("<option value='1'>未进行</option><option value='2'>缓考</option><option value='3'>缺考</option><option value='4'>考试舞弊</option>");
							 	}%>
						      </select>		
					</div>
				</div>
	...
	if(ac.equals("post")){
		...
		String sql = "update jz_achievement_mark t set name='"+name+  "',code='"+code+  "',status='"+status+"',fraction='"+fraction+"'  where t.id = "+id;
