...
<!--        <button class="layui-btn " >删除</button>-->
        <button class="layui-btn "  onclick="newDistrbutor()" >新建教学进程符号</button>
        	<%
		       	//获取文件后面的对象 数据
				...
				//查询的字段局部语句
		 		...
		 		if(search_val.length()>=1){
		 			search="where t.process_symbol_name like '%"+search_val+"%'";
		 		}else{
		 			search = "where 1=1";
		 		}
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from dict_process_symbol t   "+search;
		        ...
		        String bank_sql= "select t.*  from dict_process_symbol  t "+search+"  limit "+(pages-1)*limits+","+limits+";";

				String html_str = "";
		        ArrayList<String> list  = new ArrayList<String>();
            		ResultSet groups = db.executeQuery(bank_sql);
            		while(groups.next()){
            			String publicstate = groups.getString("publicstate");
            			if("1".equals(publicstate)){
            				publicstate = "是";
            			}else{
            				publicstate = "否";
            			}
            			html_str = "<tr>"
   							+"<td class=\"\">"+groups.getString("process_symbol_name") +"</td>          "
   							+"<td class=\"\">"+publicstate+"</td>         "
   							+"<td class=\"\">"+groups.getString("process_symbol")+"</td>	"
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="公共进程"     data-sortable="true" data-filter-control="select" data-visible="true" >公共进程</th>
              <th data-field="编码"     data-sortable="true" data-filter-control="select" data-visible="true" >符号</th>
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'new_teaching_process_symbol.jsp'
		});
    }
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'edit_teaching_process_symbol.jsp?id='+id 
		});
    } 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          ...
			          data : { id: id,table:"dict_process_symbol"},  
