...
<body>
	...
	<div id="tb" class="form_top layui-form"  style="display: flex;"><br>
       <br><input id="search" type="text" class="layui-input" style="margin-right:10px;width:50%" placeholder="请输入类别">
      					 <select id="fangshi" name="fangshi"  lay-verify="required"  class="textbox-text">
						        <option value="1" >等级方式1</option>
						        <option value="2">等级方式2</option>
						        <option value="3">等级方式3</option>
						      </select>
        <button class="layui-btn "  onclick="ac_tion('search')">搜索</button>
        <button class="layui-btn " onclick="location.reload()" > 刷新</button>
<!--        <button class="layui-btn " >删除</button>-->
        <button class="layui-btn "  onclick="newDistrbutor()" >新建成绩等级</button>
        	<%
		       	//获取文件后面的对象 数据
		       	String search_val = request.getParameter("val"); 
        		String fangshi_val = request.getParameter("fangshi");
        		System.out.println("fagns================="+fangshi_val);
        		if(fangshi_val==null){fangshi_val="1";}
        		System.out.println("fagns================="+fangshi_val);
				...
				String fangshi ="";
		 		if(search_val.length()>=1){
		 			search="where name like '%"+search_val+"%' and style_fraction="+fangshi_val;
		 		}else{
		 			search = "where 1=1 and style_fraction="+fangshi_val;
		 		}
		 		
		 		//计算出总页数
				String zpag_sql="select count(t.id)  row from jz_achievement_leve t   "+search;
		        ...
		        String bank_sql= "select t.*  from jz_achievement_leve  t "+search+"  limit "+(pages-1)*limits+","+limits+";";
		        ...
		        ArrayList<String> list  = new ArrayList<String>();
            		...
            		while(groups.next()){
            			html_str = "<tr>"
   							...
   							+"<td class=\"\">"+groups.getString("code")+"</td>         "
   							+"<td class=\"\">"+groups.getString("fraction")+"</td>         "
   							+"<td class=\"\">"+groups.getString("start_fraction")+"</td>         "
   							+"<td class=\"\">"+groups.getString("end_fraction")+"</td>         "
   							+"<td class=\"\">"+groups.getString("style_fraction")+"</td>         "
    ...
    <table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
        <thead>
            <tr>
              ...
              <th data-field="等级分数"     data-sortable="true" data-filter-control="select" data-visible="true" >等级分数</th>
              <th data-field="开始分数"     data-sortable="true" data-filter-control="select" data-visible="true" >开始分数</th>
              <th data-field="截止分数"     data-sortable="true" data-filter-control="select" data-visible="true" >截止分数</th>
              <th data-field="等级防暑"     data-sortable="true" data-filter-control="select" data-visible="true" >等级方式</th>
    ...
    function ac_tion() {
	       ...
	       window.location.href="?ac=&val="+$('#search').val()+"&fangshi="+fangshi;
	} 
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'new_grade_maintenance.jsp'
		});
    }
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  offset: 't',
		  area: ['940px', '100%'],
		  content: 'edit_grade_maintenance.jsp?id='+id 
		});
    } 
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          ...
			          data : { id: id,table:"jz_achievement_leve"},  
