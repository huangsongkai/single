...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (assessment_name like '%"+search_val+"%' or assessment_number like '%"+search_val+"%') ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_assessment "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_assessment  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         	...
         	*/
         	html_str="<tr>\r\n"+
			"<td ><strong>"+Rs.getString("assessment_name")+"</strong></td>\r\n"+//考核方式名称
			"<td ><strong>"+Rs.getString("assessment_number")+"</strong></td>\r\n"+//考核方式编码
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
						     "<button class=\"layui-btn\" onclick=\"edit_assessment_maintenance('"+Rs.getString("id")+"')\">修改</button>"+
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="display: flex;">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary" onclick="new_assessment_maintenance()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe61f;</i> 增加</button>
<%--		        <button class="layui-btn layui-btn-small  layui-btn-primary " onclick="help(<%=PMenuId %>)"  style="height: 35px;  color:#A9A9A9; background: rgb(245, 245, 245);"><i class="layui-icon" >&#xe60c;</i>帮助</button>--%>
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                <th data-field="学科类别名称"  data-sortable="true" data-filter-control="select"  data-visible="true">学科类别名称</th>
		                <th data-field="学科类别编码"  data-sortable="true" data-filter-control="select"  data-visible="true">学科类别编码</th>       	              	  
	    ...
	    <script type="text/javascript">  
		        ...
		        function new_assessment_maintenance(){
			    	 layer.open({
			    		  type: 2,
			    		  title: '添加考核方式',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_assessment_maintenance.jsp' 
			    		});
			     }
			     function edit_assessment_maintenance(id){
			    	 layer.open({
			    		  type: 2,
			    		  title: '编辑考核方式',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_assessment_maintenance.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_assessment WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./assessment_maintenance.jsp');</script>");
	   }
	   else{
