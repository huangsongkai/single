...
<%
		...
		//查询的字段局部语句
 		...
 		if(search_val.length()>=1){
 			search=search+"and (categoryname like '%"+search_val+"%' ) ";
 		}  	
       	/**
       	 *分页相关
       	 */
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from dict_subject_category "+search+";";
		int zpag= db.Row(zpag_sql);			
		 
		//SQL语句
       	String sql="select * from dict_subject_category  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
			...
			"<td ><strong>"+Rs.getString("categoryname")+"</strong></td>\r\n"+//学科类别名称
			"<td ><strong>"+Rs.getString("categorycode")+"</strong></td>\r\n"+//学科类别编码
						"<td >"+
						    "<div class=\"layui-btn-group\">"+
						     "<button class=\"layui-btn\" onclick=\"edit_discipline_class_manage('"+Rs.getString("id")+"')\">修改</button>"+
	...
	<body>
	    <div class="" style="padding-bottom: 0px;margin-left: 1%;margin-right: 1%; height:auto;">  
			<div id="tb" class="form_top layui-form" style="display: flex;">
		        ...
		        <button class="layui-btn layui-btn-small  layui-btn-primary" onclick="new_discipline_class_manage()"  style="height: 35px;  color: white; background: rgb(25, 160, 148);"> <i class="layui-icon" style="color: #FFF;">&#xe61f;</i> 增加</button>
		    </div>
	    ...
	    <script type="text/javascript">  
		        ...
		        function new_discipline_class_manage(){
			    	 layer.open({
			    		  type: 2,
			    		  title: '添加学科类别',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_discipline_class_manage.jsp' 
			    		});
			     }
			     function edit_discipline_class_manage(id){
			    	 layer.open({
			    		  type: 2,
			    		  title: '编辑学科类别',
			    		  shadeClose: true,
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_discipline_class_manage.jsp?id='+id 
			    		});
			     }
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM dict_subject_category WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除成功');window.location.replace('./discipline_class_maintenance.jsp');</script>");
	   }
	   else{
