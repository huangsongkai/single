...
<%
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from person_education_change "+search+";";
		int zpag= db.Row(zpag_sql);			

		//SQL语句
       	String sql="select * from person_education_change  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         ...
         //	"<td >"+common.idToFieidName("dict_campus","campus_name",Rs.getString("campus"))+"</td>\r\n"+//院系编号
			...
			"<td ><strong>"+Rs.getString("degree") +"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("degree_num")+"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("degree_date")+"</strong></td>\r\n"+
		        ...
		        function ac_tion() {
				 ...
				 function new_departments_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_education_change.jsp' 
			    		});
			     }
				 function edit_departments_manage(id){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_education_change.jsp?id='+id 
			    		});
			     }
	    </script>
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM person_education_change WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除变化成功');window.location.replace('./education_change.jsp');</script>");
	   }
	   else{
