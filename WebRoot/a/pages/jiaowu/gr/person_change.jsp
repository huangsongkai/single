...
<%
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from person_change "+search+";";
		int zpag= db.Row(zpag_sql);			

		//SQL语句
       	String sql="select * from person_change  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
			...
			"<td ><strong>"+Rs.getString("branch")+"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("work_info")+"</strong></td>\r\n"+
				"<td ><strong>"+Rs.getString("work_people")+"</strong></td>\r\n"+
	    	...
	    	<table class="cuoz" id="table" data-toggle="table" data-show-toggle="true" data-show-columns="true" data-show-export="true" >
		        <thead>
		            <tr>
		                ...
		                <th data-field="开始时间"  data-sortable="true" data-filter-control="select"  data-visible="true">开始时间</th>
		        ...
		        function ac_tion() {
				 ...
				 function edit_departments_manage(id){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_person_change.jsp?id='+id 
			    		});
			     }
	    </script>
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM person_change WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除变化成功');window.location.replace('./person_change.jsp');</script>");
	   }
	   else{
