...
<%
        ...
        //当前页条数
      	...
      	//计算出总页数
		String zpag_sql="select count(1) as row from person_certificate_change "+search+";";
		int zpag= db.Row(zpag_sql);			

		//SQL语句
       	String sql="select * from person_certificate_change  "+search+" order by id asc limit  "+(pages-1)*limits+","+limits+";";
        ...
        while(Rs.next()){
         ...
         //	"<td >"+common.idToFieidName("dict_campus","campus_name",Rs.getString("campus"))+"</td>\r\n"+//院系编号
			...
			"<td ><strong>"+Rs.getString("cer_name") +"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("cer_type") +"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("cer_num") +"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("cer_date") +"</strong></td>\r\n"+
			"<td ><strong>"+Rs.getString("cer_address") +"</strong></td>\r\n"+
		        ...
		        function ac_tion() {
				 ...
				 function new_departments_manage(){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'new_certificate_change.jsp' 
			    		});
			     }
				 function edit_departments_manage(id){
			    	 layer.open({
			    		  ...
			    		  shade: 0.5,
			    		  area: ['940px', '80%'],
			    		  content: 'edit_certificate_change.jsp?id='+id 
			    		});
			     }
	    </script>
...
<% if("deletet".equals(ac)){ 
	 ...
	 if(id==null){id="";}
	try{
	   String dsql="DELETE FROM person_certificate_change WHERE id='"+id+"';";
	   if(db.executeUpdate(dsql)==true){
		   out.println("<script>parent.layer.msg('删除变化成功');window.location.replace('./certificate_change.jsp');</script>");
	   }
	   else{
