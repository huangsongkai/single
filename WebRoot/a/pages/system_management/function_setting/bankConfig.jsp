...
<!--        <button class="layui-btn layui-btn-small  layui-btn-primary" >删除</button>-->
        <button class="layui-btn layui-btn-small  layui-btn-primary "  onclick="newDistrbutor()" >新建银行</button>
        	<%
		       	//获取文件后面的对象 数据
				...
				//查询的字段局部语句
		 		...
		 		if(search_val.length()>1){
		 			search="where bankName like '%"+search_val+"%'";
		 		}else{
		 			search = "where 1=1";
		 		}
		 		//计算出总页数
				String zpag_sql="select count(1) as row from g_bank "+search+";";
        ...
        <thead>
            <tr>
             <th data-field="state"  data-checkbox="true"><input type="checkbox" name="" ></th>
              ...
              <th data-field="id"     data-sortable="true" data-filter-control="select" data-visible="true" >id</th>
              <th data-field="名称"     data-sortable="true" data-filter-control="select" data-visible="true" >名称</th>
              <th data-field="银行利率"     data-sortable="true" data-filter-control="select" data-visible="true" >银行利率(a)</th>
              <th data-field="费率"     data-sortable="true" data-filter-control="select" data-visible="true" >费率(b)</th>
              <th data-field="区域"     data-sortable="true" data-filter-control="select" data-visible="true" >区域</th>
              <th data-field="创建时间"  data-sortable="true" data-filter-control="select" data-visible="true">创建时间</th>
          ...
          <%
		        ...
		        String bank_sql= "select t.*,t1.regionalname  from g_bank t,s_regional_table t1  "+search+" and t1.regionalcode=t.regioncode  limit  "+(pages-1)*10+",10;";
 System.out.println(bank_sql);	
          ...
          <tr>
	        ...
	        <td class="" ><%=i %></td>
			<td class=""><%=groups.getString("id") %></td>
			<td class=""><%=groups.getString("bankName") %></td>
			<td class=""><%=groups.getString("bankrate") %></td>
			<td class=""><%=groups.getString("rate") %></td>
			<td class=""><%=groups.getString("regionalname") %></td>
			<td class=""><%=groups.getString("createtime") %></td>
			<td class=""><a onclick="lookBank(<%=groups.getString("id") %>)">查看</a><a onclick="editBank(<%=groups.getString("id") %>)">编辑</a><a onclick="deletet(<%=groups.getString("id") %>)">删除</a></td>
	      </tr>
	      <%i++;} %>
    ...
    function newDistrbutor(){
    	layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['940px', '440px'],
		  content: 'new_bankConfig.jsp' 
		});
    }
    
    function lookBank(id){
		layer.open({
		 type: 2,
		  title: '查看银行配置',
		  shadeClose: true,
		  ...
		  shade: 0.5,
		  area: ['980px', '530px'],
		  content: 'detail_bankConfig.jsp?id='+id 
		});
     ...
     function editBank(id){
		layer.open({
		 type: 2,
		  ...
		  shade: 0.5,
		  area: ['940px', '340px'],
		  content: 'edit_bankConfig.jsp?id='+id 
		});
  ...
  layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {  
			    ...
			    var uid = '<%=Suid%>';
			  	...
			  	$.ajax({  
			         ...
			         type : "post",  
			          url : basePath+"/Api/v1/?p=web/do/doDelBank",
			          data : { id: id},  
