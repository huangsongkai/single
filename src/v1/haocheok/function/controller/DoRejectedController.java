 ...
 *
 */
public class DoRejectedController {
	Jdbc db = new Jdbc();
	Page page = new Page();
	
	public void DoRejected(HttpServletRequest request,
...
System.out.println("拒绝模块接收数据：" + RequestJson);
		...
		try {
		   ...
		   if("2".equals(status_form)){
			   ...
			   while(orderSet.next()){
				   ...
				   String insertString = "insert into `process_log` (orderid,processid,regionalcode,nodeid,rolecode," +
					...
					"creation_date,creation_uid,updatetime,up_uid) " +
					 "VALUES('3','"+processid+"','"+regionalcode+"','"+nodeid_to+"','"+map.get("rolecode")+"','"+map.get("rolename")+"','"+info.getUSERID()+"'," +
			   ...
			   if(status){
					...
					json.put("msg", "拒绝操作成功");
					//添加操作日志
					Atm.LogSys("拒绝", "用户拒绝操作", "拒绝操作者"+info.getUSERID()+"", "0",info.getUSERID(), info.getIp());
			   }else{
				   	json.put("success", "false");
					json.put("resultCode", "2000");
					json.put("msg", "拒绝操作出错");
					//添加操作日志
					Atm.LogSys("拒绝", "用户拒绝操作", "拒绝操作者"+info.getUSERID()+"", "1",info.getUSERID(), info.getIp());
			   }
