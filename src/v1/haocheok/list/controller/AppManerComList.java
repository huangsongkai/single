 ...
 *
 */
public class AppManerComList {

	public void ManerComList(HttpServletRequest request, HttpServletResponse response,String RequestJson,InfoEntity info) throws ServletException, IOException {
		...
		try {
			...
			String order_sql =  "SELECT order_customerfile.customername,order_customerfile.phonenumber,temp1.orderid AS orderid, " +
								...
								"AND temp1.rolecode = 'administration' AND temp1.status = 11 "  +
			...
			//添加操作日志
			Atm.LogSys("获取业务主管已完成列表", "用户APP端成功", "用户"+username+"获取业务主管已完成列表", "0",info.getUSERID(), info.getIp());
