 ...
 * @date:2017-8-25 下午01:51:31
 */
public class AppSalesComList {

	public void SalesComList(HttpServletRequest request, HttpServletResponse response,String RequestJson,InfoEntity info) throws ServletException, IOException {
		...
		try {
			...
			String order_sql =  "SELECT order_customerfile.customername,order_customerfile.phonenumber,temp1.orderid AS orderid, " +
								"ordercode,temp1.processid AS processid,loantype,order_sheet.createtime AS createtime FROM " +
								"(SELECT orderid,creation_date,regionalcode,STATUS,rolename,processid,rolecode FROM " +
								"(SELECT orderid,creation_date,regionalcode,STATUS,rolename,processid,rolecode FROM process_log ORDER BY creation_date DESC ) " +
								...
								"AND temp1.rolecode = 'administration' AND temp1.status = 6 "  +
			...
			while(orderPr.next()){
				...
				json_order.put("orderstatus", "放款完成");
			...
			//添加操作日志
			Atm.LogSys("获取业务员已完成列表", "用户APP端成功", "用户"+username+"获取业务员已完成列表", "0",info.getUSERID(), info.getIp());
