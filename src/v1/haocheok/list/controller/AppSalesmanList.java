 ...
 * @date:2017-8-25 下午01:52:31
 */
public class AppSalesmanList {

	public void SalesmanList(HttpServletRequest request, HttpServletResponse response,String RequestJson,InfoEntity info) throws ServletException, IOException {
		...
		try {
			...
			String order_sql =  "SELECT order_customerfile.customername,order_customerfile.phonenumber,temp1.orderid AS orderid, " +
								"ordercode,temp1.processid AS processid,loantype,order_sheet.createtime AS createtime,temp1.creation_date as creation_date FROM " +
								...
								"AND order_sheet.salesmanid = '"+info.getUSERID()+"' " +
...
System.out.println("order_sql"+order_sql);
			...
			while(orderPr.next()){
System.out.println("时间测试======="+DateUtils.getDatePoor_fen(orderPr.getTimestamp("creation_date"),orderPr.getTimestamp("createtime")));
				...
				json_order.put("orderstatus", "外检师正在办理中");
				json_order.put("timeconsum", DateUtils.getDatePoor_fen(orderPr.getTimestamp("creation_date"),orderPr.getTimestamp("createtime")));
			...
			//添加操作日志
			Atm.LogSys("获取业务员办理中列表", "用户APP端成功", "用户"+username+"获取业务员办理中列表", "0",info.getUSERID(), info.getIp());
