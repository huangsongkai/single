...
public class SelectIdByPath {

	private HttpServletRequest request;
	private HttpServletResponse response;

	public SelectIdByPath (HttpServletRequest request, HttpServletResponse response) {
		   ...
		   try { // 解析开始
			   ...
			   for (int i = 0; i < arr.size(); i++) {
				   JSONObject obj = arr.getJSONObject(i);
				    //定义json解析字段列表
				   attachmentpath= obj.get("attachmentpath") + "";
	  ...
	  try {   
	            ...
	            String regiona="SELECT attachmentid FROM order_attachment where attachmentpath='"+attachmentpath+"'";
	              ...
	              while(regionaa.next()){
	  				...
	  				+ Dpage
	  				+ "\",\"pages\":\""+Zpages2+"\",\"Count\":\""+Zongshu+"\",\"attachmentid\":"+regionaa.getInt("attachmentid")+"}";
	  		   
	              }
 
			  if (regionaa != null) { regionaa.close(); }
