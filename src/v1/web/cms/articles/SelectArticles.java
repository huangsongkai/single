...
public class SelectArticles {
	...
	public SelectArticles (HttpServletRequest request, HttpServletResponse response) {
	...
	public void Transmit(String Token,String UUID,String USERID,String DID,String Mdels,String NetMode,String ChannelId,String RequestJson,String ip,String GPS,String GPSLocal,String AppKeyType,long TimeStart) throws ServletException, IOException {
	  ...
	  try {   
	          ...
	          //去栏目表查找用户权限
              String SQLW="SELECT id,releasepeople FROM `cms_class` WHERE depth=1";
              ResultSet RSW = db.executeQuery(SQLW);
              String releasepeople=null;
              while (RSW.next()){
            	  releasepeople=RSW.getString("releasepeople");
              ...
              String fidss=fids.substring(0, fids.length()-1);
              //分页 search
				 int   listnum2=Integer.parseInt(listnum);;
				 int   Asum2=db.Row(" SELECT COUNT(*) as row FROM `cms_articles`  WHERE one_class_id in ("+fidss+")");
				 int Zongshu=Asum2;
			...
			//数据库	    
              String SQL=" SELECT * FROM `cms_articles`  WHERE one_class_id in ("+fidss+") ORDER BY addtime desc LIMIT "+DQcount2+", "+listnum2+" ";	  
