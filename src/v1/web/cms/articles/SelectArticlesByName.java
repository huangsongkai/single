...
public class SelectArticlesByName {

	private HttpServletRequest request;
	private HttpServletResponse response;

	public SelectArticlesByName (HttpServletRequest request, HttpServletResponse response) {
...
//用于删除栏目判断该栏目是否被占用
	public void Transmit(String Token,String UUID,String USERID,String DID,String Mdels,String NetMode,String ChannelId,String RequestJson,String ip,String GPS,String GPSLocal,String AppKeyType,long TimeStart) throws ServletException, IOException {
	  ...
	  try {   
	          ...
	          String SQL=" SELECT * FROM `cms_articles` where two_class_id='"+id+"' ";
		     ...
		     // System.out.print(RS.next());
		      if(RS.next())
			  {
			      ...
			      String SQLLL=" SELECT * FROM `cms_class` where fid= "+id+" ";	    
			      ResultSet RSLL = db.executeQuery(SQLLL);
			      if(RSLL.next())
				  {
					    json.put("id", ""+11);
	                    list.add(json.toString());    
