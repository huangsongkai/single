...
import v1.haocheok.enums.ProcessCode;

/**
 * web端 驳回操作
 * @author zhoukai04171019
 * @date:2017-9-11 上午11:09:38
 */
public class DoReject {
	Jdbc db = new Jdbc();
	Page page = new Page();
	
	public void dobohui(HttpServletRequest request,
		...
		try {
		   ...
		   //1.驳回操作
		   if(ProcessCode.Reject.getReasonPhrase().equals(status_form)){
//			   String basesql = "SELECT * FROM t_yewudian , t_yewuxian WHERE t_yewuxian.nodeid_form = '"+nodeid+"' and t_yewuxian.status_form = '"+status_form+"' and t_yewudian.nodeid = t_yewuxian.nodeid_form AND t_yewudian.t_yewumian_id = '"+processid+"' AND rolecode = '"+rolecode+"';";
			...
			//添加操作日志
			Atm.LogSys("web驳回", "用户驳回操作", "驳回操作者"+info.getUSERID()+"", "0",info.getUSERID(), info.getIp());
