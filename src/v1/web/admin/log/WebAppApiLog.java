 ...
 * @version 创建时间：2017-6-25 下午23:18:32 
 * 类说明 WEB-admin-系统管理-接口执行日志-数据接口
 */
...
public class WebAppApiLog {
	...
	public void webAppApiLog(HttpServletRequest request, HttpServletResponse response,InfoEntity info,String Token,String UUID,String USERID,String DID,String Mdels,String NetMode,String ChannelId,String RequestJson,String ip,String GPS,String GPSLocal,String AppKeyType,long TimeStart) throws ServletException, IOException {
            ...
            System.out.println("接口执行日志-WEB数据接口 UUID="+UUID);
			//判断过滤非法字符: 
		    if(!regex(UUID)){ 
			...
			//处理关键词
			if(!"0".equals(keyword)){
				TJkeyword=" and (classname like '"+"%"+page.mysqlCode(keyword).replaceAll(" ", "%")+"%' or UUID like '"+"%"+page.mysqlCode(keyword).replaceAll(" ", "%")+"%')"; //过滤整合关键词查询
			}
			...
			//总拼装查询提交语句
			 ...
			 int Asum2=db.Row("SELECT COUNT(1) as row FROM  ec_appuse_log  "+whereString);
			 int Zongshu=Asum2;
		    ...
		    //主查询语句
			String LogSql = "SELECT id,UUID,uid,did,Mdels,NetMode,classpath,classname,ip,ExeTime,logintime  FROM  ec_appuse_log "+whereString+" order by id desc   LIMIT "+DQcount2+", "+listnum2+"  ";
			...
			try {
					...
					while (Rs.next()) {
						 ...
						 json.put("did", Rs.getString("did")); 
						 json.put("Mdels",Rs.getString("Mdels"));
						 json.put("NetMode", ""+Rs.getString("NetMode"));
						 json.put("classpath", ""+Rs.getString("classpath"));
						 json.put("classname",  ""+Rs.getString("classname"));
						 json.put("ip", ""+Rs.getString("ip")); 
						 json.put("ExeTime", ""+Rs.getString("ExeTime"));
						 json.put("logintime", ""+Rs.getString("logintime"));
